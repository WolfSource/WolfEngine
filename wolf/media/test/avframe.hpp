...
BOOST_AUTO_TEST_CASE(avframe_test) {
  ...
  w_av_config _dst_config = {AVPixelFormat::AV_PIX_FMT_BGRA, _config.width, _config.height};
  ...
  const auto _new_path = std::filesystem::current_path().append("/rgb_converted.png");
  ...
  std::cout << "avframe_test is done" << std::endl;
}
