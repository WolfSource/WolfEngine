...
BOOST_AUTO_TEST_CASE(av1_encode_decode_test) {
  ...
  boost::leaf::try_handle_all(
      [&]() -> boost::leaf::result<void> {
        ...
        BOOST_REQUIRE(_encode_result >= 0);
        ...
        const auto _new_path =
            std::filesystem::current_path().append("/av1_decoded.png");
...
BOOST_AUTO_TEST_CASE(vp9_encode_decode_test) {
  ...
  boost::leaf::try_handle_all(
      [&]() -> boost::leaf::result<void> {
        ...
        const auto _new_path =
            std::filesystem::current_path().append("/vp9_decoded.png");
        BOOST_LEAF_AUTO(final_result, final_frame.save_to_img_file(_new_path));
...
BOOST_AUTO_TEST_CASE(h264_encode_decode_test) {
  const wolf::system::w_leak_detector _detector = {};

  std::cout << "h264_encode_decode_test is running" << std::endl;
  ...
  boost::leaf::try_handle_all(
      [&]() -> boost::leaf::result<void> {
        ...
        const auto _new_path =
            std::filesystem::current_path().append("/h264_decoded.png");
        BOOST_LEAF_AUTO(final_result, final_frame.save_to_img_file(_new_path));
  ...
  std::cout << "h264_encode_decode_test just done" << std::endl;
}
