...
boost::leaf::result<int> w_decoder::decode(_In_ const w_av_packet &p_packet,
  ...
  for (;;) {
    const auto _bytes = av_parser_parse2(
        this->ctx.parser, this->ctx.codec_ctx, &_dst_packet._packet->data,
        &_dst_packet._packet->size, p_packet._packet->data,
        p_packet._packet->size, AV_NOPTS_VALUE, AV_NOPTS_VALUE, 0);
