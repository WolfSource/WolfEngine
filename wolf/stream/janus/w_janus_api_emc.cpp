...
w_janus_api_emc::init(_In_ int p_debug_mode,
  ...
  return EM_ASM_INT({
    ...
    // create a session
					...
					janus.attach(
						{
							...
							onlocaltrack: function (p_track, p_on) {
								...
								if (p_track.kind === "audio") {
									...
									$('#streamscreen').append('<video class="rounded centered" id="screenvideo' + _track_id + '" width=100% autoplay playsinline muted="muted"/>');
      ...
      new_remote_feed = function (p_id, display) {
	      ...
	      janus.attach({
	      		...
	      		success: function (pluginHandle) {
	      			...
	      			_remote_feed.send({ message: listen });
	      		},
	      		error: function (error) {
	      		...
	      		iceState: function (state) {
	      			Janus.log("ICE state (feed #" + _remote_feed.rfindex + ") changed to " + state);
	      		},
	      		webrtcState: function (on) {
	      			Janus.log("Janus says this WebRTC PeerConnection (feed #" + _remote_feed.rfindex + ") is " + (on ? "up" : "down") + " now");
	      		},
	      		...
	      		onmessage: function (msg, jsep) {
	      			...
	      			Janus.debug("Janus event: " + event);
	      			if (event) {
	      				if (event === "attached") {
	      					// Subscriber created and attached
	      					if (!spinner) {
	      						var target = document.getElementById('#streamscreen');
	      					...
	      					Janus.error("unexpected point of program");
	      				}
	      			}
	      			if (jsep) {
	      				...
	      				_remote_feed.createAnswer(
	      					{
	      						...
	      						success: function (jsep) {
	      							...
	      							_remote_feed.send({ message: body, jsep: jsep });
	      						},
	      						error: function (error) {
	      		...
	      		onremotetrack: function (track, mid, on) {
	      			...
	      			if (track.kind === "audio") {
	      				...
	      				remote_tracks[mid] = stream;
	      				Janus.log("Created remote audio stream:", stream);
	      				$('#streamscreen').append('<audio class="hide" id="screenvideo' + mid + '" playsinline/>');
	      				...
	      				$('#streamscreen .no-video-container').remove();
	      				stream = new MediaStream([track]);
	      				_remote_feed.remote_tracks[mid] = stream;
	      				Janus.log("Created remote video stream:", stream);
	      				$('#streamscreen').append('<video class="rounded centered" id="screenvideo' + mid + '" width=100% playsinline/>');
	      				$('#screenvideo' + mid).get(0).volume = 0;
	      				Janus.attachMediaStream($('#screenvideo' + mid).get(0), stream);
	      				$('#screenvideo' + mid).get(0).play();
	      				$('#screenvideo' + mid).get(0).volume = 1;
