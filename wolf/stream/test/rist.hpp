      ...
      [&]() -> boost::leaf::result<void> {
        ...
        auto _on_frame_cb = [t0](const w_av_packet &p_packet,
          ...
          if (_stream_index == p_video_stream_index) {
            ...
            auto _res = _decoder->decode(p_packet, _decoded_frame, true);
            if (_res.has_error()) {
              return false; // close it
            }
          ...
          const auto t1 = std::chrono::high_resolution_clock::now();
          if (std::chrono::duration_cast<std::chrono::seconds>(t1 - t0) > 50s) {
            return false;
          }
          return true;
        };
        ...
        auto _opt =
            std::vector<w_av_set_opt>{w_av_set_opt{"rist_profile", "main"}};
        constexpr auto _url = "rist://@127.0.0.1:1236";
