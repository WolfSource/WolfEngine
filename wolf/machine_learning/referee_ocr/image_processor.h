...
/*! \brief This class is responsible for image processing tasks.
	...
	This class contains functions, structures, and variables that use for image
   processing purposes. In the project, in other classes, in the case of
...
/*!
	...
	\param  ocr_config    The necessary configurations for processing optical
	characters.
	*/
void gaussian_blur(
	_Inout_ cv::Mat &frame_box,
	_In_ config_for_ocr_struct &ocr_config);

/*!
	takes an image and makes the background white. this function uses a threshold to find background points
	\param  contour_image    The image needs to be processed.
	\param  ocr_config    The necessary configurations for processing optical
	characters.
	*/
void make_contour_white_background(
	_Inout_ cv::Mat &contour_image,
	_In_ config_for_ocr_struct &ocr_config);

/*!
	  The negative_image function changed the pixels' value. The new value is obtained by 255 - the previous value.

	  \param  contour_image    contour_image is a cropped part of the original image that contains one of the contours.
	*/
void negative_image(
	_Inout_ cv::Mat &contour_image);
...
/*!
	  ...
	  \param  ocr_config    The necessary configurations for processing optical characters.
	*/
