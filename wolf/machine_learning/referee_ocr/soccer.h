...
/*! \brief It is extract the soccer game result.
	...
	This class contains functions, structures, and variables that use the SOCCER class
  ...
  public:
	...
	*/
	...
	The function return the config of desired image window.
		...
		\return
	*/
	W_SYSTEM_EXPORT static auto set_config(
		_In_ char* pType) -> OCR::config_struct;
	...
	*/
	...
	W_SYSTEM_EXPORT auto set_config_for_ocr(
		_In_ char* pType) -> config_for_ocr_struct;

	/*!
		The function returns frame results, the results are extracted from pre-defined frame boxes.
	...
	*/
	...
	/*!
		The function returns frame results, the results are extracted based on the character clusters' symmetricity.
	...
	*/
	...
	/*!
		The function checks for penalty results and if the scene contains the penalty results the results would be extracted.
	...
	*/
	...
	/*!
	The function returns the game results, if the image contain the game final result.

		\param pRawImage The sequence of the input image pixels array in BGR format.
		\param height The image height.
		\param width The image width.
		\param pStr The image result.
		\return Void
	*/
	W_SYSTEM_EXPORT auto
	single_image_result_extraction(
		_In_ uint8_t* pRawImage,
		_In_ int height,
		_In_ int width,
		_In_ ocr_callback* callback) -> int;

	/*!
	The extract_all_image_char_clusters function returns the character cluster related to the frame result.
	...
	*/
	W_SYSTEM_EXPORT auto extract_all_image_char_clusters(
		...
		std::vector<std::vector<OCR::characters_struct>>& pTimeCandidates) -> void;

	/*!
	replace team names stored in match_result_struct using string similarity algorithms.

		\param result input struct
		\return
	*/
	W_SYSTEM_EXPORT static auto replace_team_names_with_most_similar_string(
		_Inout_ std::vector<ocr_referee_class::match_result_struct>& result) -> void;

	/*!
	The initial_match_result_struct function fills a match_result_struct with the initial values.

		\param frame_data input struct
		\param image input struct
		\return An initialed variable of match data structure.
	*/
	W_SYSTEM_EXPORT auto initial_match_result_struct(
		ocr_referee_class::frame_result_struct frame_data,
		cv::Mat& image) -> ocr_referee_class::match_result_struct;
	...
	The update_match_data function store frames data in the match_date variable.
		...
		\return
	*/
	W_SYSTEM_EXPORT auto update_match_data(
		_In_ ocr_referee_class::frame_result_struct frame_data,
		_In_ cv::Mat& image) -> void;

	/*!
	The extract_game_results function extracts the game results from the match_data
	and stores the results in the match_data.
	...
	*/
	W_SYSTEM_EXPORT auto get_matches_data() -> std::vector<ocr_referee_class::match_result_struct>;
