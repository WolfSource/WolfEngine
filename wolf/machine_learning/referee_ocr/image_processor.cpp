	...
	_In_ config_for_ocr_struct &ocr_config)
{
	...
	cv::GaussianBlur(frame_box, frame_box, cv::Size(kernel_size, kernel_size), 0, 0);
}
	...
	if (ocr_config.make_white_background)
	{
		if (n_channels == 1)
		{
			for (int i = 0; i < height; i++)
			{
				...
				{
					if (contour_image.at<uchar>(i, j) > ocr_config.white_background_threshold)
					{
		...
		else if (n_channels == 3)
		{
			for (int i = 0; i < height; i++)
			{
				for (int j = 0; j < width; j++)
				{
					...
					if (count > 2 || combine > ocr_config.white_background_threshold * 2 + 100)
					{
						color_pixel[0] = 255;  // (color_pixel[0]*2 > 255) ? 255:color_pixel[0];
						color_pixel[1] = 255;  // (color_pixel[1]*2 > 255) ? 255:color_pixel[1];
						color_pixel[2] = 255;  // (color_pixel[2]*2 > 255) ? 255:color_pixel[2];
					}
				}
			}
		}
	...
	_In_ config_for_ocr_struct &ocr_config)
{
	if (frame_box.channels() == 3)
	{
		if (ocr_config.binary)
		{
			...
			cv::Mat kernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3), cv::Point(0, 0));
		...
		else
		{
			cv::inRange(
				frame_box,
				cv::Scalar(
					ocr_config.threshold_value, ocr_config.threshold_value, ocr_config.threshold_value),
				cv::Scalar(255, 255, 255), frame_box);
		}
	}
	else if (frame_box.channels() == 1)
	{
		cv::threshold(frame_box, frame_box, ocr_config.threshold_value, 255, cv::THRESH_BINARY);
	}
	// return image;
}
