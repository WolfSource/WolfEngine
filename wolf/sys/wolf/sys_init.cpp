...
{
#ifdef WIN32
    std::array<wchar_t, W_MAX_PATH> _buffer = { 0 };
    GetModuleFileNameW(nullptr, _buffer.data(), sizeof(_buffer));
    return std::filesystem::path(_buffer.data(), std::filesystem::path::auto_format).parent_path();
#else
    std::string _str;
...
int w_sys_init(_Inout_ char* p_buf, _In_ size_t p_buf_len)
{
