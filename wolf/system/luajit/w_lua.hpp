...
namespace wolf::system::luajit {
    using w_lib = sol::lib;

    class w_lua {
    public:

        W_API w_lua() : _load_result(nullptr, -1, 0, 0, sol::load_status::syntax) {}
        ...
        */
        ...
        W_API void open_libraries(ARGS &&...p_libs) {
          _lua.open_libraries(std::forward<ARGS>(p_libs)...);
        }
        ...
        */
        W_API boost::leaf::result<void> load_file(const std::filesystem::path &p_file) {
          _load_result = _lua.load_file(p_file.string());
        ...
        */
        ...
        W_API boost::leaf::result<std::tuple<RETYPES...>>
        run_file(const std::filesystem::path &p_file) {
          auto result = _lua.script_file(p_file.string());
          if (!result.valid()) {
            sol::error err = result;
            return W_FAILURE(1, err.what());
          }
        ...
        */
        template<typename ...RETYPES>
        W_API boost::leaf::result<std::tuple<RETYPES...>>
        run_from_buffer(const std::string_view p_str) {
          auto result = _lua.script(p_str);
          if (!result.valid()) {
            sol::error err = result;
            return W_FAILURE(1, err.what());
          }
        ...
        */
        template<typename ...ARGS>
        W_API boost::leaf::result<void> bind_to_function(const std::string &p_lua_function, ARGS &&...p_args) {
          try {
            _lua.set_function(p_lua_function, std::forward<ARGS>(p_args)...);
          } catch (...) {
        ...
        */
        template<typename ...RETYPES, typename ...ARGS>
        W_API boost::leaf::result<std::tuple<RETYPES...>>
        call_function(const std::string_view p_name, ARGS &&...p_args) {
          sol::protected_function function = _lua[p_name];
        ...
        */
        W_API boost::leaf::result<sol::state &> get_state() {
          return _lua;
        };
        ...
        */
        ...
        W_API boost::leaf::result<void> set_global_variable(std::string_view p_name, ARGS &&...p_value) {
          try {
            _lua.set(p_name, std::forward<ARGS>(p_value)...);
          } catch (...) {
