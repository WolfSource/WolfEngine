...
struct w_trace {
  ...
  w_trace(_In_ const stack &p_stack) { this->_stacks.emplace_front(p_stack); }
...
constexpr boost::leaf::result<T> W_OK(T &p_param) {
    return boost::leaf::result<T>(std::move(p_param));
}

inline boost::leaf::result<void> W_OK() { return boost::leaf::result<void>(); }
