...
namespace Wolf
{
	namespace Graphics
	{
		...
		{
		public:
			API W_Mesh(const std::shared_ptr<Graphics::W_GraphicsDevice>& pGDevice);
			API virtual ~W_Mesh();

			//Load a Identity quad
			API HRESULT Load(std::wstring pVertexShaderPath, std::wstring pPixelShaderPath, std::wstring pTexture2DPath, bool pLoadDeafultTexture2D,
				const Wolf::Graphics::VertexTypes::VertexPositionNormalTexture* const pVertices, const UINT pVerticesSize, 
				const unsigned short* const pIndices, const UINT pIndicesSize);
			//initialize default texture and assign to material of mesh
			API HRESULT LoadDeafultTexture(UINT pWidth, UINT pHeight, D3D11_USAGE pUsage = D3D11_USAGE_DEFAULT, D3D11_CPU_ACCESS_FLAG pCpuAccess = D3D11_CPU_ACCESS_WRITE);
...
#pragma region Setters
			...
			void					SetPosition(DirectX::XMFLOAT3 pValue)				{ this->position = pValue; }
			//Set rotation of mesh
			void					SetRotation(DirectX::XMFLOAT3 pValue)				{ this->rotation = pValue; }
			//Set scale of mesh
			void					SetScale(DirectX::XMFLOAT3 pValue)					{ this->scale = pValue; }
			//Set view matrix
			void					SetView(DirectX::XMMATRIX pValue)					{ this->view = pValue; }
			//Set projection matrix
			void					SetProjection(DirectX::XMMATRIX pValue)				{ this->projection = pValue; }
			//Set uv
			void					SetUVScale(DirectX::XMFLOAT2 pUVScale)				{ this->objInfoCBuffer.Const.UVScale = pUVScale; }
			//Set current frame of sprite
			void					SetSpriteCurrentFrame(UINT pCurrentFrame)			{ this->spriteObjCBuffer.Const.CurrentFrame = static_cast<float>(pCurrentFrame); }
			//Set number of sprites per row
			void					SetSpritesPerRow(UINT pSpritesPerRow)				{ this->spriteObjCBuffer.Const.SpritesPerRow = static_cast<float>(pSpritesPerRow); }
			//Set number of sprites per column
			void					SetSpritesPerColumn(UINT pSpritesPerColumn)			{ this->spriteObjCBuffer.Const.SpritesPerCol = static_cast<float>(pSpritesPerColumn); }
			//Enable/Disable using identity matrix
			void					SetUseIdentityMatrix(bool pValue)					{ this->useIdentityMatrix = pValue; }
