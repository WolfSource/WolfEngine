...
W_Mesh::W_Mesh(const shared_ptr<W_GraphicsDevice>& pGDevice) : gDevice(pGDevice), useDefaultTexture(false),
stride(sizeof(VertexPositionNormalTexture)), offset(0), useIdentityMatrix(false), isSprite(false)
{
	this->Name = typeid(this).name();

	this->texture2D = make_unique<W_Texture2D>();
	this->defaultTexture = make_unique<W_Texture2D>();
...
HRESULT W_Mesh::Load(std::wstring pVertexShaderPath, std::wstring pPixelShaderPath, const VertexPositionNormalTexture* const pVertices, const UINT pVerticesSize,
...
HRESULT W_Mesh::LoadDeafultTexture(UINT pWidth, UINT pHeight)
{
	auto device = this->gDevice->device.Get();
	//Create default texture
	auto hr = this->defaultTexture->Create(device, pWidth, pHeight);
	hr = this->texture2D->Create(device, pWidth, pHeight);
	return hr;
}
...
void W_Mesh::Render(ID3D11ShaderResourceView* srv)
{
	auto context = this->gDevice->context;
	{
		...
		{
			this->shader->SetSRV(0, 1, this->defaultTexture->GetSRV());
		}
		else
		{
...
ULONG W_Mesh::Release()
{
	...
	SMART_RELEASE(this->texture2D);
	SMART_RELEASE(this->defaultTexture);
	SMART_RELEASE(this->shader);
