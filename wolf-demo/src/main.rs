...
use wolf::{
    system::{chrono::gametime::GameTime, os::sigslot::SigSlot},
    w_log,
};
...
#[cfg(target_arch = "wasm32")]
use wolf::stream::webrtc::janus::Janus;
...
//     RunTime::sleep_async(std::time::Duration::from_secs(2)).await;
...
// // wait for threads
// RunTime::sleep_async(std::time::Duration::from_secs(1)).await;
...
pub fn app() {
    ...
    Janus::init("all".to_owned(), |p_init_state| {
        if p_init_state {
            Janus::join_room(2815533724190121.0, "pooya".to_owned());
            // Janus::share_screen(
            //     "pooya".to_owned(),
            //     "pooya's room".to_owned(),
            //     500_000,
            //     |p_room_id: f64| {
            //         w_log!("screen {} just shared", p_room_id);
            //     },
            // );
        }
    })
