...
void scene::load()
{
	...
	if (_hr == W_FAILED)
	{
		...
		V(W_FAILED,
			w_log_type::W_ERROR,
			true,
			"creating wireframe pipeline. graphics device: {} . trace info: {}", _gDevice->get_info(), _trace_info);
	}
...
W_RESULT scene::render(_In_ const wolf::system::w_game_time& pGameTime)
{
		...
		false) == W_FAILED)
	{
		...
		V(W_FAILED,
			w_log_type::W_ERROR,
			true,
			"submiting queue. graphics device: {} . trace info: {}", _gDevice->get_info(), _trace_info);
	}
