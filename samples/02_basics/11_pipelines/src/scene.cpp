...
void scene::load()
{
    ...
    auto _rasterization_states = wolf::graphics::w_graphics_device::defaults::vk_default_pipeline_rasterization_state_create_info;
	...
	{
		release();
		V(S_FALSE, "creating wireframe pipeline", _trace_info, 3, true);
	}
	...
	for (uint32_t i = 0; i < _size; ++i)
	{
		this->_draw_command_buffers.begin(i);
		{
				...
				0.0f);
			{
                ...
                if (_show_wireframe)
                {
                    this->_wireframe_pipeline.bind(_cmd, &_description_set);
                }
                else
                {
                     this->_solid_pipeline.bind(_cmd, &_description_set);
                }
                //++++++++++++++++++++++++++++++++++++++++++++++++++++
                //++++++++++++++++++++++++++++++++++++++++++++++++++++
...
HRESULT scene::render(_In_ const wolf::system::w_game_time& pGameTime)
{
    ...
    //submit two gui and draw command buffers
	if (_gDevice->submit(
