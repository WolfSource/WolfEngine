...
//++++++++++++++++++++++++++++++++++++++++++++++++++++
static int randi(const int& pMin, const int& pMax)
{
    return static_cast<int>(pMin + rand() % (pMax - pMin + 1));
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++
//++++++++++++++++++++++++++++++++++++++++++++++++++++
...
void scene::load()
{
    ...
    if (_hr == W_FAILED)
    {
        ...
        V(W_FAILED, 
			w_log_type::W_ERROR,
			true,
			"loading staging texture. graphics device: {} . trace info: {}", _gDevice->get_info(), _trace_info);
    }
    ...
    if (_hr == W_FAILED)
    {
        ...
        V(W_FAILED, 
			w_log_type::W_ERROR,
			true,
			"initializing staging texture buffer. graphics device: {} . trace info: {}", _gDevice->get_info(), _trace_info);
    }
