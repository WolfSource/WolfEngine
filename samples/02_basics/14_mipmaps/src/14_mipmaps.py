...
class scene(QWidget):
    ...
    def load(self):
        ...
        #++++++++++++++++++++++++++++++++++++++++++++++++++++
        global _gui
        self._texture_max_lod = self._texture.get_mip_maps_level()
        _gui.slider.setMaximum(self._texture_max_lod)
        _gui.slider.setValue(0)
        #++++++++++++++++++++++++++++++++++++++++++++++++++++
        #++++++++++++++++++++++++++++++++++++++++++++++++++++

        #load shader uniform
        _hr = self._u0.load(self._gDevice, 8) # 4 bytes for texture_lod(float) and 4 bytes for sampler_index(float)
        if _hr:
        ...
        _hr = self._u0.update([self._texture_lod, self._sampler_index])
        if _hr:
        ...
        _shader_param_0.type = pyWolf.graphics.w_shader_binding_type.UNIFORM
        _shader_param_0.stage = pyWolf.graphics.w_shader_stage_flag_bits.VERTEX_SHADER
        _shader_param_0.buffer_info = self._u0.get_descriptor_info()

        #++++++++++++++++++++++++++++++++++++++++++++++++++++
	    #The following codes have been added for this project
	    #++++++++++++++++++++++++++++++++++++++++++++++++++++
        _image_descriptor_info = self._texture.get_descriptor_info(pyWolf.graphics.w_sampler_type.MIPMAP_AND_ANISOTROPY)
	    #++++++++++++++++++++++++++++++++++++++++++++++++++++
	    #++++++++++++++++++++++++++++++++++++++++++++++++++++

        _shader_param_1 = pyWolf.graphics.w_shader_binding_param()
        _shader_param_1.index = 1
        _shader_param_1.type = pyWolf.graphics.w_shader_binding_type.SAMPLER2D
        _shader_param_1.stage = pyWolf.graphics.w_shader_stage_flag_bits.FRAGMENT_SHADER
        _shader_param_1.image_info = _image_descriptor_info
    ...
    def update(self, pGameTime):
        ...
        #The following codes have been added for this project
        #++++++++++++++++++++++++++++++++++++++++++++++++++++
        _slider_data = _gui.slider.value()
        if self._texture_lod != _slider_data:
            self._texture_lod = _slider_data
            #update uniform
            _hr = self._u0.update([self._texture_lod, self._sampler_index])
            if _hr:
                print "Error on updating uniform"
        #++++++++++++++++++++++++++++++++++++++++++++++++++++
        #++++++++++++++++++++++++++++++++++++++++++++++++++++
