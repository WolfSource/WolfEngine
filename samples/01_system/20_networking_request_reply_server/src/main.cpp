/*
	...
	Description		 : This sample shows how to create one two way client in Wolf Engine
					   make sure launch 21_networking_request_reply_client simultaneously
...
WOLF_MAIN()
{
    ...
    _log_config.app_name = L"20_networking_request_reply_server";
	...
	on_bind_established += [](const int& pSocketID)
	{
		...
		while (true)
		{
			...
			if (_rcv_len >= 0)
			{
				...
				//send ack
				std::string _msg = "server: " + w_time_span::now().to_string();
    ...
    w_network _req_rep_server;
	_req_rep_server.setup_request_reply_server(
		"tcp://127.0.0.1:1238",
		on_bind_established);
