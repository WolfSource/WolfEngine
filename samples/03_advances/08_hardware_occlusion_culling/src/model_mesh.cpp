...
W_RESULT model_mesh::submit_compute_shader()
{
	...
	if (this->use_last_lod || (this->lods_states.size() && !this->lods_states[0].x))
	{
		_cam_pos.x += 10000;//set camera to far for first ref model
	...
	case 1:
		...
		_hr = this->_cs.unifrom_x1->update(_cmd);
		break;
	case 2:
...
W_RESULT model_mesh::_create_buffers(_In_ const w_command_buffer& pCommandBuffer)
{
	...
	//load indirect draws
	if (this->_indirect_draws.load(this->gDevice, pCommandBuffer, _draw_counts) == W_FAILED)
	{
		V(W_FAILED,
			w_log_type::W_ERROR,
...
W_RESULT model_mesh::_create_lod_levels_buffer(_In_ const w_command_buffer& pCommandBuffer)
{
	...
	_Inout_ std::wstring& pComputeShaderPath)
{
	...
	this->lods_states.resize(this->visibilities.size());
...
ULONG model_mesh::release()
{
	...
	this->_cs.release();
	this->lods_info.clear();
	this->visibilities.clear();
	this->lods_states.clear();
...
bool model_mesh::get_is_use_last_lod() const
{
	return this->use_last_lod;
}

bool model_mesh::get_lod_states(_In_ const uint32_t& pModelInstanceIndex) const
{
	auto _size = this->lods_states.size();

	uint32_t _base_index = pModelInstanceIndex % 4;
	uint32_t _sec_index = pModelInstanceIndex - _base_index * 4;

	return pModelInstanceIndex < _size ?
		this->lods_states[_base_index][_sec_index] == 1.0f : false;
}
...
void model_mesh::set_all_use_last_lod(_In_ const bool& pValue)
{
	this->use_last_lod = pValue;
}

void model_mesh::set_lod_states(_In_ const bool& pValue, _In_ const uint32_t& pModelInstanceIndex)
{
	uint32_t _base_index = pModelInstanceIndex % 4;
	...
	{
		uint32_t _sec_index = pModelInstanceIndex - _base_index * 4;
		this->lods_states[_base_index][_sec_index] = pValue ? 1.0f : 0.0f;
	}
}
