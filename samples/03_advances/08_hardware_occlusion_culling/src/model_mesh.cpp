...
#pragma endregion 
		...
		//add first lod which is mesh
		lod_info _lod_info;
		_lod_info.first_index = 0;
		_lod_info.index_count = _batch_indices.size();
		this->_lods_info.push_back(_lod_info);

		//the second lod isn bounding box of mesh
		this->_merged_bounding_box.generate_vertices();
		for (size_t i = 0; i < W_ARRAY_SIZE(this->_merged_bounding_box.vertices); ++i)
		{
			_batch_vertices.push_back(this->_merged_bounding_box.vertices[i]);
			_batch_indices.push_back(_base_vertex_index + i);
		}
		
		_lod_info.first_index = _lod_info.index_count;
		_lod_info.index_count = _batch_indices.size() - _lod_info.index_count;
		this->_lods_info.push_back(_lod_info);
...
#pragma endregion
	...
	W_RESULT draw(_In_ const w_command_buffer& pCommandBuffer)
	{
		...
		//The following codes have been added for this project
		//++++++++++++++++++++++++++++++++++++++++++++++++++++
		auto _lod_info = this->_draw_lod ? &this->_lods_info[1] : &this->_lods_info[0];
		return this->_mesh->draw(pCommandBuffer, nullptr, 0, 0, nullptr, 0, 
...
#pragma region Setters
	...
	void set_draw_lod(_In_ const bool& pValue)
	{
		this->_draw_lod = pValue;
	}
...
{
	return !this->_pimp ? W_FAILED : this->_pimp->draw(pCommandBuffer);
}
...
void model_mesh::set_draw_lod(_In_ const bool& pValue)
{
	if (this->_pimp)
	{
		this->_pimp->set_draw_lod(pValue);
	}
}
