...
#pragma region Getters
	...
	bool													get_is_use_last_lod() const;
	bool													get_lod_states(_In_ const uint32_t& pModelInstanceIndex = 0) const;
	wolf::render::vulkan::w_semaphore*						get_compute_semaphore();
	compute_stage_output									get_result_of_compute_shader();
	bool													get_is_sky() const { return this->_is_sky; }
...
#pragma region Setters
	...
	void set_all_use_last_lod(_In_ const bool& pValue);
	void set_lod_states(_In_ const bool& pValue, _In_ const uint32_t& pModelInstanceIndex);
...
protected:
	...
	std::vector<glm::vec4>											lods_states;
private:
