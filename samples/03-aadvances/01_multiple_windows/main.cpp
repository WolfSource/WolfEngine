...
int WINAPI WinMain(HINSTANCE pHInstance, HINSTANCE pPrevHInstance, PSTR pSTR, int pCmdshow)
{
	...
	logger.initialize(L"02_Multiple_Windows_DX11_X.Win32", wolf::system::io::get_current_directoryW());
	...
	*/
	auto _msg_proc_func_0 = [](HWND pHWND, UINT pMsg, WPARAM pWParam, LPARAM pLParam) -> HRESULT
	{
		switch (pMsg)
		{
		case WM_CREATE:
		...
		case WM_KEYUP:
		{
			...
			{
				sWindow_0->close();
				logger.write(L"The windows 0 just closed");
			}
		}
		break;
		}
	...
	auto _msg_proc_func_1 = [](HWND pHWND, UINT pMsg, WPARAM pWParam, LPARAM pLParam) -> HRESULT
	{
		switch (pMsg)
		{
		case WM_CREATE:
		{
			logger.write(L"The window 1 just created");
		}
		break;
		//close window on KeyUp event of Escape button
		case WM_KEYUP:
