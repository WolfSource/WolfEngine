...
pub fn new(p_cluster_name: &str, p_node_id: u64) -> MemRaft {
    let config = Arc::new(
        Config::build(p_cluster_name.into()).validate().expect(
            format!(
                "failed to build Raft config for cluster:{} and node id:{}",
                p_cluster_name, p_node_id
            )
            .as_str(),
        ),
    );
    ...
    raft::Raft::new(p_node_id, config, network, storage)
}
