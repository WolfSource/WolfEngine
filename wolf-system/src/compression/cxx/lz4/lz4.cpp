#include "lz4.hpp"
#include "wolf-system/src/compression/lz4.rs.h"
...
namespace lz4_cxx
{
        ...
        rust::String &pTrace)
    {
        ...
        auto _max_dst_size = LZ4_compressBound(_src_size);
        auto data = (char *)malloc(_max_dst_size);
        if (!data)
        {
...
#pragma clang diagnostic pop
        ...
        if (_compressed_buffer_size > 0)
        {
            ...
            data = (char *)realloc(data, _compressed_buffer_size);
            if (!data)
            {
        ...
        rust::String &pTrace)
    {
        ...
        size_t _destination_capacity = _src_size * 2;
        auto data = (char *)malloc(_destination_capacity);
        if (!data)
        {
        ...
        while (_number_of_try > 0)
        {
            ...
            else
            {
                ...
                data = (char *)realloc(data, _destination_capacity);
                if (!data)
                {
