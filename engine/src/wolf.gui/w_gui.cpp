...
std::map<std::string, std::vector<w_gui_vertex>>            w_gui::_widgets_vertices;
...
HRESULT w_gui::load(const std::wstring& pGuiDesignPath, _In_ const UINT pParentWidth, _In_ const UINT pParentHeight)
{
    ...
    _parent_width = pParentWidth;
    _parent_height = pParentHeight;
...
void w_gui::get_all_controls_id(_In_z_ const char* pWidgetName, _Inout_ std::vector<INT64>& pControlsID)
{
    auto _find = _widgets.find(pWidgetName);
    if (_find != _widgets.end())
    {
