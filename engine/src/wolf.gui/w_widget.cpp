...
HRESULT w_widget::render(_In_ const float pElapsedTime)
{
    ...
    if (!this->_minimized)
    {
        if (this->_redrawing)
        {
            this->_vertex_declarations.clear();
    ...
    else
    {
        if (this->_redrawing)
        {
            for (auto it = this->_controls.cbegin(); it != this->_controls.cend(); ++it)
            {
                ...
                (*it)->render(pElapsedTime, this->_vertex_declarations);
            }

            if (s_pControlFocus && s_pControlFocus->get_parent_widget() == this)
            {
                s_pControlFocus->render(pElapsedTime, this->_vertex_declarations);
            }
        }
    }
...
std::vector<w_gui_vertex> w_widget::get_vertex_declarations()
{
    return this->_vertex_declarations;
}
