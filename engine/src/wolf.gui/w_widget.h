...
//			void clear_comboBox(_In_ int ID);
			...
			HRESULT set_default_element(_In_ const UINT nControlType, _In_ const UINT iElement, _In_ const w_element* pElement);
...
#pragma region Getters
			...
			static w_control* 			        get_next_control(_In_ w_control* pControl);
			static w_control*                   get_prev_control(_In_ w_control* pControl);
            ...
            //W_EXP w_image*						get_image(_In_ const int pID) const									{ return reinterpret_cast<w_image*>(get_control(pID, W_GUI_CONTROL_IMAGE)); }
			//get button control by ID
            //W_EXP w_button*					    get_button(_In_ const int pID) const								{ return reinterpret_cast<w_button*>(get_control(pID, W_GUI_CONTROL_BUTTON)); }
			//get checkbox control by ID
            //W_EXP w_check_box*					get_checkBox(_In_ const int pID) const								{ return reinterpret_cast<w_check_box*>(get_control(pID, W_GUI_CONTROL_CHECKBOX)); }
			//get radioButton control by ID
            //W_EXP w_radio_button*				get_radio_button(_In_ const int pID) const							    { return reinterpret_cast<w_radio_button*>(get_control(pID, W_GUI_CONTROL_RADIOBUTTON)); }
			//get comboBox control by ID
            //W_EXP w_combo_box*					get_combo_box(_In_ const int pID) const								{ return reinterpret_cast<w_combo_box*>(get_control(pID, W_GUI_CONTROL_COMBOBOX)); }
			//get slider control by ID
            //W_EXP w_slider*					    get_slider(_In_ const int pID) const								{ return reinterpret_cast<w_slider*>(get_control(pID, W_GUI_CONTROL_SLIDER)); }
			//get list_widget control by ID
            //W_EXP w_list_widget*				get_list_widget(_In_ const int ID) const							    { return reinterpret_cast<w_list_widget*>(get_control(ID, W_GUI_CONTROL_LISTWIDGET)); }
...
#pragma region Events
			 ...
			 std::function<void(const w_point&)>                        on_mouse_move_callback;
			 std::function<void(const w_point&)>                        on_mouse_enter_callback;
			 std::function<void(const w_point&)>                        on_mouse_leave_callback;
...
#pragma endregion
            ...
            static std::vector<w_widget*>                               widgets;
