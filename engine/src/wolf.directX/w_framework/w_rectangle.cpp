...
void w_monitor::printf(_In_ w_sprite_batch* pSpriteBatch, const w_game_time& pGameTime, _In_ DirectX::XMFLOAT2* const pPosition)
{
	this->cpuInfo->update();
...
ULONG w_monitor::release()
{
	...
	COM_RELEASE(this->_lime_solid_brush_color);
	COM_RELEASE(this->_yellow_solid_brush_color);
	COM_RELEASE(this->_red_solid_brush_color);
