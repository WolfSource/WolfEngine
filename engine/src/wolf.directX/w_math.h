...
//Global defines
#ifndef RGBA_TO_DWORD_COLOR
#define RGBA_TO_DWORD_COLOR(r,g,b,a) \
...
namespace DirectX
{
	inline XMFLOAT4 DWORD_COLOR_TO_XMFLOAT4(DWORD pC)
	{
	...
	inline W_COLOR DWORD_COLOR_TO_W_COLOR(DWORD pC)
	{
		//Convert ARGB(DWORD) to RGBA(in the range of 0..255)
		return W_COLOR(
			((pC >> 16) & 0xFF),//R
			((pC >> 8) & 0xFF),//G
			(pC & 0xFF),//B
	...
	inline XMFLOAT4 W_COLOR_TO_XMFLOAT4(W_COLOR pC)
	{
		return DirectX::XMFLOAT4(pC.r / 255.0f, pC.g / 255.0f, pC.b / 255.0f, pC.a / 255.0f);
	}

	inline W_COLOR XMFLOAT4_TO_W_COLOR(XMFLOAT4 pC)
	{
	...
	inline XMMATRIX XMMatrixFromGLMMatrix(glm::tmat4x4<float, glm::precision::highp> const& pValue)
	{
		return XMMATRIX(
