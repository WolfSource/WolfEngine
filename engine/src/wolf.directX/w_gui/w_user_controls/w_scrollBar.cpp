...
w_scrollBar::w_scrollBar(_In_opt_ w_widget* pParent) : _super(pParent),
_scroll_color(RGBA_TO_DWORD_COLOR(200, 200, 200, 255)),
_scroll_background_color(RGBA_TO_DWORD_COLOR(255, 255, 255, 200)),
_scroll_disabled_color(RGBA_TO_DWORD_COLOR(255, 255, 255, 70))
{
...
w_scrollBar::~w_scrollBar()
{
}

void w_scrollBar::update_rects()
{
	_super::update_rects();
...
void w_scrollBar::update_thumb_rect()
{
...
void w_scrollBar::scroll(_In_ int nDelta)
{
	// Perform scroll
...
void w_scrollBar::show_item(_In_ int nIndex)
{
	// Cap the index
...
bool w_scrollBar::handle_keyboard(UINT pMsg, WPARAM pWParam, LPARAM pLParam)
{
	W_UNUSED(pMsg);
	W_UNUSED(pWParam);
	W_UNUSED(pLParam);
...
_Use_decl_annotations_
bool w_scrollBar::handle_mouse(UINT pMsg, const POINT& pPoint, WPARAM pWParam, LPARAM pLParam)
{
	W_UNUSED(pWParam);
	W_UNUSED(pLParam);
	...
	auto _hwnd = w_widgets_resource_manager::get_HWND();
...
_Use_decl_annotations_
bool w_scrollBar::on_msg_proc(UINT pMsg, WPARAM pWParam, LPARAM pLParam)
{
	W_UNUSED(pWParam);
...
void w_scrollBar::render(const std::shared_ptr<wolf::graphics::w_graphics_device>& pGDevice, _In_ float pElapsedTime)
{
...
void w_scrollBar::set_track_range(_In_ int pStart, _In_ int pEnd)
{
	this->start = pStart;
	this->end = pEnd;
	cap();
...
void w_scrollBar::set_track_pos(_In_ int pPosition)
{
...
void w_scrollBar::set_page_size(_In_ int pPageSize)
{
