...
w_comboBox::w_comboBox(_In_opt_ w_widget* pParent) : scrollBar(pParent), _super(pParent),
scrollBar_width(16),
is_opened(false),
m_nDropHeight(100),
m_iSelected(-1),
m_iFocused(-1),
_item_height(21),
_comboBox_main_color(RGBA_TO_DWORD_COLOR(200, 200, 200, 150)),
_comboBox_main_pressed_color(RGBA_TO_DWORD_COLOR(255, 255, 255, 200)),
_comboBox_main_mouse_over_color(W_COLOR::YELLOW),
_comboBox_main_focused_color(RGBA_TO_DWORD_COLOR(255, 255, 0, 70)),
_comboBox_main_disabled_color(RGBA_TO_DWORD_COLOR(200, 200, 200, 70)),
_comboBox_button_color(RGBA_TO_DWORD_COLOR(255, 255, 255, 150)),
_comboBox_button_pressed_color(RGBA_TO_DWORD_COLOR(150, 150, 150, 255)),
_comboBox_button_mouse_over_color(W_COLOR::YELLOW),
_comboBox_button_focused_color(RGBA_TO_DWORD_COLOR(255, 255, 255, 200)),
_comboBox_button_disabled_color(RGBA_TO_DWORD_COLOR(255, 255, 255, 70)),
_comboBox_drop_down_color(RGBA_TO_DWORD_COLOR(255, 255, 255, 255)),
_comboBox_selected_font_color(W_COLOR(255, 255, 255, 255)),
_comboBox_selected_color(1, 0, 0, 0.6f),
_comboBox_scroll_color(W_COLOR(200, 200, 200, 255)),
_comboBox_scroll_background_color(W_COLOR(255, 255, 255, 200)),
_comboBox_scroll_disabled_color(W_COLOR(255, 255, 255, 70))
{
	...
	_super::_label_color = RGBA_TO_DWORD_COLOR(0, 0, 0, 255);
	_super::_label_pressed_color = RGBA_TO_DWORD_COLOR(0, 0, 0, 255);
	_super::_label_mouse_over_color = RGBA_TO_DWORD_COLOR(0, 0, 0, 255);
	_super::_label_focused_color = RGBA_TO_DWORD_COLOR(0, 0, 0, 255);
	_super::_label_disabled_color = RGBA_TO_DWORD_COLOR(200, 200, 200, 200);
}

w_comboBox::~w_comboBox()
{
	remove_all_items();
}

HRESULT w_comboBox::on_initialize(const std::shared_ptr<wolf::graphics::w_graphics_device>& pGDevice)
{
	return _super::parent_widget->initialize_control(&this->scrollBar);
}

void w_comboBox::remove_all_items()
{
...
void w_comboBox::update_rects()
{
	...
	this->rectangle_drop_down.bottom += m_nDropHeight;
	...
	this->scrollBar.set_location(this->rectangle_drop_down.right, this->rectangle_drop_down.top + 2);
	this->scrollBar.set_size(this->scrollBar_width, _rect_height - 2);
	auto pFontNode = _super::parent_widget->get_manager()->get_font_node(this->elements[2]->index_font);
	if (pFontNode && pFontNode->height)
	{
		this->scrollBar.set_page_size(/*RectHeight*/(this->rectangle_drop_down_text.bottom - this->rectangle_drop_down_text.top) / pFontNode->height);

		// The selected item may have been scrolled off the page.
		// Ensure that it is in page again.
		this->scrollBar.show_item(m_iSelected);
	}
}

void w_comboBox::on_focus_out()
{
	_super::on_focus_out();
...
bool w_comboBox::handle_keyboard(UINT pMsg, WPARAM pWParam, LPARAM pLParam)
{
	...
	if (this->scrollBar.handle_keyboard(pMsg, pWParam, pLParam)) return true;
...
_Use_decl_annotations_
bool w_comboBox::handle_mouse(UINT pMsg, const POINT& pPoint, WPARAM pWParam, LPARAM pLParam)
{
	if (!_super::enabled || !_super::visible) return false;

	// Let the scroll bar handle it first.
	if (this->scrollBar.handle_mouse(pMsg, pPoint, pWParam, pLParam)) return true;
	...
	case WM_LBUTTONDBLCLK:
	{
		...
		{
			// Determine which item has been selected
			for (size_t i = this->scrollBar.get_track_pos(); i < this->items.size(); i++)
			{
				auto pItem = this->items[i];
				if (pItem->visible && PtInRect(&pItem->active, pPoint))
				{
	...
	case WM_MOUSEWHEEL:
	{
		...
		if (this->is_opened)
		{
			...
			this->scrollBar.scroll(-zDelta * uLines);
		}
		else
		{
			if (zDelta > 0)
...
bool w_comboBox::can_have_focus()
{
	return (_super::visible && _super::enabled);
}

void w_comboBox::render(const std::shared_ptr<wolf::graphics::w_graphics_device>& pGDevice, _In_ float pElapsedTime)
{
	...
	if (!bSBInit)
	{
		...
		{
			this->scrollBar.set_page_size((this->rectangle_drop_down_text.bottom - this->rectangle_drop_down_text.top) / _super::parent_widget->get_manager()->get_font_node(_element->index_font)->height);
		}
		else
		{
			this->scrollBar.set_page_size(this->rectangle_drop_down_text.bottom - this->rectangle_drop_down_text.top);
		}
		bSBInit = true;
	}
	...
	if (this->is_opened)
	{
		this->scrollBar.set_scroll_color(this->_comboBox_scroll_color);
		this->scrollBar.set_scroll_background_color(this->_comboBox_scroll_background_color);
		this->scrollBar.set_scroll_disabled_color(this->_comboBox_scroll_disabled_color);

		this->scrollBar.render(pGDevice, pElapsedTime);
	}

	_element->texture_color.color_states[W_GUI_STATE_NORMAL] = this->_comboBox_drop_down_color;
	...
	_selection_element->texture_color.current_color_state = this->_comboBox_selected_color;
	...
	for (size_t i = this->scrollBar.get_track_pos(), index = 0; i < _items_size; i++, index++)
	{
		auto pItem = this->items[i];
		...
		if (this->is_opened)
		{
			...
			if ((int) i == m_iFocused)
			{
				...
				_super::text_offset_y = _height_over_2 + (int)index * this->_item_height + _y_step;

				w_label::current_color_state = this->_comboBox_selected_font_color;
	...
	_element->texture_color.color_states[W_GUI_STATE_NORMAL] = this->_comboBox_button_color;
	_element->texture_color.color_states[W_GUI_STATE_MOUSEOVER] = this->_comboBox_button_mouse_over_color;
	_element->texture_color.color_states[W_GUI_STATE_PRESSED] = this->_comboBox_button_pressed_color;
	_element->texture_color.color_states[W_GUI_STATE_FOCUS] = this->_comboBox_button_focused_color;
	_element->texture_color.color_states[W_GUI_STATE_DISABLED] = this->_comboBox_button_disabled_color;
	...
	_element->texture_color.color_states[W_GUI_STATE_NORMAL] = this->_comboBox_main_color;
	_element->texture_color.color_states[W_GUI_STATE_FOCUS] = this->_comboBox_main_focused_color;
	_element->texture_color.color_states[W_GUI_STATE_DISABLED] = this->_comboBox_main_disabled_color;
...
_Use_decl_annotations_
HRESULT w_comboBox::add_item(const WCHAR* strText, void* pData)
{
	// Validate parameters
	if (!strText) return E_INVALIDARG;

	// Create a new item and set the data
	auto pItem = new (std::nothrow) w_comboBoxItem;
	if (!pItem)
	{
	...
	ZeroMemory(pItem, sizeof(w_comboBoxItem));
	wcscpy_s(pItem->text, 256, strText);
	pItem->data = pData;
...
void w_comboBox::remove_item(_In_ UINT pIndex)
{
...
bool w_comboBox::contains_item(_In_z_ const WCHAR* pText, _In_ UINT pIndexStart)
{
	return (-1 != find_item(pText, pIndexStart));
}

int w_comboBox::find_item(_In_z_ const WCHAR* pText, _In_ UINT pIndexStart) const
{
	if (!pText) return -1;
...
void* w_comboBox::get_item_data(_In_z_ const WCHAR* pText) const
{
...
void* w_comboBox::get_item_data(_In_ int pIndex) const
{
...
void* w_comboBox::get_selected_data() const
{
...
w_comboBoxItem* w_comboBox::get_selected_item() const
{
	if (m_iSelected < 0) return nullptr;
...
void w_comboBox::set_drop_height(_In_ UINT pHeight)
{
	m_nDropHeight = pHeight;
	update_rects();
}

void w_comboBox::set_scrollBar_width(_In_ int pWidth)
{
	this->scrollBar_width = pWidth;
	update_rects();
}

HRESULT w_comboBox::set_selected_by_index(_In_ UINT pIndex)
{
...
HRESULT w_comboBox::set_selected_by_text(_In_z_ const WCHAR* pText)
{
...
HRESULT w_comboBox::set_selected_by_data(_In_ void* pData)
{
	for (size_t i = 0; i < this->items.size(); i++)
	{
		auto pItem = this->items[i];
