...
w_listBox::w_listBox(_In_opt_ w_widget* pParent) : _super(pParent),
scrollBar(pParent),
_text_margin_x(0),
_text_margin_y(0),
_icon_margin_x(0),
...
_listBox_disabled_color(RGBA_TO_DWORD_COLOR(255, 255, 255, 155)), 
...
HRESULT w_listBox::on_initialize(const std::shared_ptr<wolf::graphics::w_graphics_device>& pGDevice)
{
...
bool w_listBox::can_have_focus() 
{
	return _super::visible && _super::enabled;
}

void w_listBox::update_rects()
{
	_super::update_rects();
	...
	this->scrollBar.set_location(_super::boundingBox.right - this->scrollBar_Width, _super::boundingBox.top);
...
HRESULT w_listBox::add_item(_In_z_ const std::wstring& pText, _In_z_ const std::wstring& pIconPath, _In_ bool pRelateivePath, _In_opt_  void* pData)
{
	auto pNewItem = new (std::nothrow) w_listBoxItem();
...
_Use_decl_annotations_
HRESULT w_listBox::insert_item(int pIndex, _In_z_ const std::wstring& pText, _In_z_ const std::wstring& pIconPath, _In_ bool pRelateivePath, _In_opt_ void* pData)
{
	auto pNewItem = new (std::nothrow) w_listBoxItem;
	if (!pNewItem) return E_OUTOFMEMORY;
...
void w_listBox::remove_item(_In_ int pIndex)
{
...
void w_listBox::remove_all_items()
{
...
w_listBoxItem* w_listBox::get_item(_In_ int nIndex) const
{
...
int w_listBox::get_selected_index(_In_ int pPreviousSelected) const
{
...
void w_listBox::select_item(_In_ int pNewIndex)
{
...
_Use_decl_annotations_
bool w_listBox::handle_keyboard(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
_Use_decl_annotations_
bool w_listBox::handle_mouse(UINT uMsg, const POINT& pt, WPARAM wParam, LPARAM lParam)
{
	if (!_super::enabled || !_super::visible) return false;
...
_Use_decl_annotations_
bool w_listBox::on_msg_proc(UINT pMsg, WPARAM pWParam, LPARAM pLParam)
{
	W_UNUSED(pWParam);
	...
	if (WM_CAPTURECHANGED == pMsg)
	{
		...
		if ((HWND) pLParam != w_widgets_resource_manager::get_HWND())
		{
			this->drag = false;
		}
	}
...
void w_listBox::render(const std::shared_ptr<wolf::graphics::w_graphics_device>& pGDevice, _In_ float pElapsedTime)
{
	...
	if (!this->items.empty())
	{
		...
		for (int i = this->scrollBar.get_track_pos(); i < (int) this->items.size(); ++i)
		{
			...
			if (bSelectedStyle)
			{
				...
				if (pItem->icon)
				{
					pItem->icon->set_location(_rectangle.left + this->_icon_margin_x, _rectangle.top + this->_icon_margin_y);
