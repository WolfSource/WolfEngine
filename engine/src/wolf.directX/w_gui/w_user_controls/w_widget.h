...
#include <w_gui/w_texture_node.h>
...
namespace wolf
{
	namespace gui
	{
		...
		public:
			DX_EXP w_widget(HWND pHwnd, int pParentWindowWidth, int pParentWindowHeight);
			DX_EXP ~w_widget();
			...
			DX_EXP void initialize(const std::shared_ptr<wolf::graphics::w_graphics_device>& pGDevice,
				...
				w_widgets_resource_manager* pManager,
				bool pRegisterDialog = true);

			DX_EXP void initialize(const std::shared_ptr<wolf::graphics::w_graphics_device>& pGDevice,
				wolf::graphics::w_sprite_batch* pSpriteBatch,
				w_widgets_resource_manager* pManager,
				bool pRegisterDialog, 
			...
			/*
				...
				pID = Unique ID of control, please do not use INT_MAX and INT_MIN, (-2147483648) and 2147483647 are reserved,
				pText = Text,
			...
			*/
			DX_EXP HRESULT add_check_box(int pID,
				_In_z_ std::wstring pText,
				_In_ UINT pWidth, _In_ UINT pHeight,
				_In_ int pX, _In_ int pY,
			...
			/*
				...
				pCreatedControl = Output reference created checkBox
			*/
			DX_EXP HRESULT add_radio_button(int pID,
				UINT pButtonGroup,
				_In_z_ std::wstring pText,
				_In_ UINT pWidth, _In_ UINT pHeight,
				_In_ int pX, _In_ int pY,
...
#pragma region Getters
			...
			DX_EXP void							get_position(_Out_ UINT& pX, _Out_ UINT& pY)      					{ pX = this->_x; pY = this->_y; }
			...
			DX_EXP w_label*						get_label(_In_ int pID) const										{ return reinterpret_cast<w_label*>(get_control(pID, W_GUI_CONTROL_LABEL)); }
			//get image control by ID
			DX_EXP w_image*						get_image(_In_ int pID) const										{ return reinterpret_cast<w_image*>(get_control(pID, W_GUI_CONTROL_IMAGE)); }
			//get button control by ID
			DX_EXP w_button*					get_button(_In_ int pID) const										{ return reinterpret_cast<w_button*>(get_control(pID, W_GUI_CONTROL_BUTTON)); }
			//get checkbox control by ID
			DX_EXP w_check_box*					get_checkBox(_In_ int pID) const									{ return reinterpret_cast<w_check_box*>(get_control(pID, W_GUI_CONTROL_CHECKBOX)); }
			//get radioButton control by ID
			DX_EXP w_radio_button*				get_radio_button(_In_ int pID) const								{ return reinterpret_cast<w_radio_button*>(get_control(pID, W_GUI_CONTROL_RADIOBUTTON)); }
			//get comboBox control by ID
			DX_EXP w_combo_box*					get_combo_box(_In_ int pID) const									{ return reinterpret_cast<w_combo_box*>(get_control(pID, W_GUI_CONTROL_COMBOBOX)); }
			//get slider control by ID
			DX_EXP w_slider*					get_slider(_In_ int pID) const										{ return reinterpret_cast<w_slider*>(get_control(pID, W_GUI_CONTROL_SLIDER)); }
			//get listBox control by ID
			DX_EXP w_list_box*					get_listBox(_In_ int ID) const										{ return reinterpret_cast<w_list_box*>(get_control(ID, W_GUI_CONTROL_LISTBOX)); }
...
#pragma region Setters
			...
			//set enabled
			DX_EXP  void	set_enabled(_In_ bool pValue)															{ this->_enabled = pValue; }
			//set z index order
...
#pragma endregion
		...
		private:
			...
			//shared brush over controls of this widget
			...
			std::vector<int>											_textures_indices;
			// Index into m_FontCache
			std::vector<int>											_fonts_indices;
