...
HRESULT w_sprite_font::load(_In_ const std::shared_ptr<wolf::graphics::w_graphics_device>& pGDevice)
{
	...
	{
		_hr = set_color(pGDevice->context_2D.Get(), ColorF::White);
	}
...
{
	std::memset(&pBoundingBox, -1, sizeof(D2D1_RECT_F));
...
{
	auto _hr = pContext->CreateSolidColorBrush(color, &this->_brush);
