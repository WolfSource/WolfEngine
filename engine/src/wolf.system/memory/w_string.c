    ...
    _In_ const char* pData)
{
    ...
    *pStringView = NULL;
    ...
    if (_len)
    {
        *pStringView = (w_string_t*)w_malloc(pMemPool, sizeof(w_string_t));
        if (*pStringView)
        {
            size_t _size = _len + 1;
            (*pStringView)->data = (char*)w_malloc(pMemPool, _size); //with NULL
    ...
    _Inout_ w_string pSrc)
{
    ...
    {
        (*pDst)->data = (char*)w_malloc(pMemPool, _len + 1); //with NULL
