/*
	Project			 : Wolf Engine. Copyright(c) Pooya Eimandar (http://PooyaEimandar.com) . All rights reserved.
	Source			 : Please direct any bug to https://github.com/PooyaEimandar/Wolf.Engine/issues
	Website			 : http://WolfSource.io
	Name			 : wolf_version.h
	Description		 : represent build version of wolf.engine
	Comment          :
*/

#if _MSC_VER > 1000
#pragma once
#endif

#ifndef __WOLF_VERSION_H__
#define __WOLF_VERSION_H__

#define WOLF_MAJOR_VERSION 1	// Making incompatible API changes
#define WOLF_MINOR_VERSION 68	// Adding functionality in a backwards - compatible manner
#define WOLF_PATCH_VERSION 0	// bug fixes
#define WOLF_DEBUG_VERSION 1	// for debugging

#endif //__WOLF_VERSION_H__


