...
namespace SimplygonSDK
{
	...
	static bool LoadOSLibrary( std::basic_string<TCHAR> path )
		...
		if( !DynamicLibraryIsLoaded() || hDLL == NULL )
			{
	...
	bool LicenseData::IsLicenseForPlatform() const
		...
		if( this->Platform != std::string("SDK") )
			return false;
		if( this->Version != std::string("8.0") )
			return false;
		return true;
		}
	...
	static bool LoadLicense( bool only_common_folder , LicenseData &lic )
		...
		if( !only_common_folder )
			...
			LicensePath = GetApplicationPath() + _T("Simplygon_8_license.dat");
		...
		lic.Clear();
		LicensePath = GetAppDataPath() + _T("Simplygon_8_license.dat");
		if( lic.ReadLicense( LicensePath ) )
			{
			if( lic.IsLicenseForPlatform() )
				{
		...
		lic.Clear();
		LicensePath = GetCommonAppPath() + _T("Simplygon_8_license.dat");
		if( lic.ReadLicense( LicensePath ) )
			{
			if( lic.IsLicenseForPlatform() )
				{
		...
		lic.Clear();
		LicensePath = GetInstallationPath() + _T("Simplygon_8_license.dat");
		if( lic.ReadLicense( LicensePath ) )
			{
			if( lic.IsLicenseForPlatform() )
				{
		...
		for( size_t i=0; i<AdditionalSearchPaths.size(); ++i )
			{
			LicensePath = AdditionalSearchPaths[i] + _T("Simplygon_8_license.dat");
			if( lic.ReadLicense( LicensePath ) )
				{
				if( lic.IsLicenseForPlatform() )
					{
	...
	int InitializeSimplygonSDK( SimplygonSDK::ISimplygonSDK **pInterfacePtr , LPCTSTR SDKPath , LPCTSTR LicenseDataText )
		...
		if( retval == 0 )
			{
			char buffer[400];
			sprintf_s(buffer, sizeof(buffer), _T("Simplygon initialized and running.\n\tVersion: %s\n"), (*pInterfacePtr)->GetVersion());
	...
	LPCTSTR GetError( int error_code )
		...
		switch( error_code ) 
			...
			case SG_ERROR_LICENSE_WRONGAPPLICATION:
				return _T("the license is not made for this application of Simplygon");
			case SG_ERROR_LICENSE_WRONGPLATFORM:
				return _T("the license is not made for this platform of Simplygon");
			case SG_ERROR_LICENSE_WRONGVERSION:
				return _T("the license is not made for this version of Simplygon");
			default:
				return _T("Extended Internal Error Code. Contact support@simplygon.com for help");
			}
		}
