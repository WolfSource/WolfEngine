...
int GDIWMOpen(void)
{
    ...
    GDIWMClass.lpszClassName = L"GDIWMClass";
...
HWND GDIWMCreateWindow(const char* name, GDIWindow* window)
{
    ...
    auto _wname = std::wstring(_name.begin(), _name.end());

    // Create the application's window
    HWND hWnd = CreateWindow(L"GDIWMClass", _wname.c_str(),
                             WS_OVERLAPPEDWINDOW,
...
int GDIWMClose(void)
{
    UnregisterClass(L"GDIWMClass", GDIWMClass.hInstance);
    return 0;
}
