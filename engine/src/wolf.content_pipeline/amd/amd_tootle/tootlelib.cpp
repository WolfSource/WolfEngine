                                             ...
                                             unsigned int*       pnFaceMapOut)
{
    ...
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : FindFaceMappingFromIndex: nIndex is invalid");
        return TOOTLE_INVALID_ARGS;
    }

    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : FindFaceMappingFromIndex: nFaces is invalid");
        return TOOTLE_INVALID_ARGS;
    }
                                             ...
                                             TootleVCacheOptimizer eVCacheOptimizer)
{
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVCache: Invalid value of nFaces");
    ...
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVCache: Invalid value of nVertices");
    ...
    if (nCacheSize == 0)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVCache: nCacheSize = 0");
    ...
    switch (eVCacheOptimizer)
    {
        ...
        case TOOTLE_VCACHE_DIRECT3D:
            ...
            logger.write("[AMD TOOTLE Debugger] Flag: DEBUG - Message : TootleOptimizeVCache: No Direct3D support for this version");
        ...
        default:
            logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVCache: Invalid eVCacheOptimizer");
                                                 ...
                                                 unsigned int*       pnFaceRemapOut)
{
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVCacheDirect3D: Invalid value of nFaces");
    ...
    if (nVertices == 0 || nVertices > TOOTLE_MAX_VERTICES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVCacheDirect3D: Invalid value of nVertices");
    ...
    if (nCacheSize == 0)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVCacheDirect3D: nCacheSize = 0");

        return TOOTLE_INVALID_ARGS;
    }

    auto pFaceRemap = new uint32_t[nFaces];
                                                ...
                                                unsigned int*       pnFaceRemapOut)
{
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVCacheLStrips: Invalid value of nFaces");
    ...
    if (nVertices == 0 || nVertices > TOOTLE_MAX_VERTICES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVCacheLStrips: Invalid value of nVertices");
    ...
    if (nCacheSize == 0)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVCacheLStrips: nCacheSize = 0");
                                              ...
                                              unsigned int*       pnFaceRemapOut)
{
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVCacheTipsy: Invalid value of nFaces");
    ...
    if (nVertices == 0 || nVertices > TOOTLE_MAX_VERTICES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVCacheTipsy: Invalid value of nVertices");
    ...
    if (nCacheSize == 0)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVCacheTipsy: nCacheSize = 0");
                                          ...
                                          unsigned int*       pnFaceRemapOut)
{
    ...
    if (nVertices == 0 || nVertices > TOOTLE_MAX_VERTICES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleClusterMesh: Invalid value of nVertices");
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleClusterMesh: Invalid value of nFaces");
    ...
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleClusterMesh: nVBStride less than 3*sizeof(float)");
                                                               ...
                                                               float               fAlpha)
{
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleFastOptimizeVCacheAndClusterMesh: Invalid value of nFaces");
    ...
    if (nVertices == 0 || nVertices > TOOTLE_MAX_VERTICES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleFastOptimizeVCacheAndClusterMesh: Invalid value of nVertices");
    ...
    if (nCacheSize == 0)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVCacheAndClusterMesh: nCacheSize = 0");
                                               ...
                                               TootleOverdrawOptimizer eOverdrawOptimizer)
{
    ...
    if (nVertices == 0 || nVertices > TOOTLE_MAX_VERTICES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeOverdraw: Invalid value of nVertices");
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeOverdraw: Invalid value of nFaces");
    ...
    if (nVBStride < 3 * sizeof(float))
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeOverdraw: nVBStride less than 3*sizeof(float)");
    ...
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeOverdraw: Invalid face winding.");
    ...
    switch (eOverdrawOptimizer)
    {
...
#ifdef _SOFTWARE_ONLY_VERSION
            logger.warning("[AMD TOOTLE Debugger] Flag: WARNING - Message : TootleOptimizeOverdraw: No Direct3D support for this version");
            return TOOTLE_INTERNAL_ERROR;
#endif
        ...
        default:
            logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeOverdraw: eOverdrawOptimizer is invalid");
                                                              ...
                                                              unsigned int*           pnClusterRemapOut)
{
    ...
    if (nVertices == 0 || nVertices > TOOTLE_MAX_VERTICES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeOverdrawDirect3D: Invalid value of nVertices");
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeOverdrawDirect3D: Invalid value of nFaces");
    ...
    if (eFrontWinding != TOOTLE_CCW && eFrontWinding != TOOTLE_CW)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeOverdrawDirect3D: Invalid face winding");
    ...
    if (!bOrdered)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeOverdrawDirect3D: Cluster array is not ordered");
                                                            ...
                                                            unsigned int*       pnClusterRemapOut)
{
    ...
    if (nVertices == 0 || nVertices > TOOTLE_MAX_VERTICES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeOverdrawFastApproximation: nVertices is invalid");
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeOverdrawFastApproximation: nFaces is invalid");
    ...
    if (nVBStride < 3 * sizeof(float))
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeOverdrawFastApproximation: nVBStride less than 3*sizeof(float)");
    ...
    if (eFrontWinding != TOOTLE_CCW && eFrontWinding != TOOTLE_CW)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeOverdrawFastApproximation: Invalid face winding");
                                       ...
                                       TootleOverdrawOptimizer eOverdrawOptimizer)
{
    ...
    if (nVertices == 0 || nVertices > TOOTLE_MAX_VERTICES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimize: Invalid value of nVertices");
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimize: Invalid value of nFaces");
    ...
    if (nCacheSize == 0)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimize: nCacheSize = 0");
    ...
    if (nVBStride < 3 * sizeof(float))
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimize: nVBStride is less than 3*sizeof(float)");
    ...
    if (eFrontWinding != TOOTLE_CCW && eFrontWinding != TOOTLE_CW)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : Invalid face winding");
                                           ...
                                           float               fAlpha)
{
    ...
    if (nVertices == 0 || nVertices > TOOTLE_MAX_VERTICES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleFastOptimize: Invalid value of nVertices");
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleFastOptimize: Invalid value of nFaces");
    ...
    if (nVBStride < 3 * sizeof(float))
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleFastOptimize: nVBStride is less than 3*sizeof(float)");
    ...
    if (nCacheSize == 0)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleFastOptimize: nCacheSize = 0");
                                             ...
                                             TootleVCacheOptimizer eVCacheOptimizer)
{
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleVCacheClusters: Invalid value of nFaces");
    ...
    if (nVertices == 0 || nVertices > TOOTLE_MAX_VERTICES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleVCacheClusters: Invalid value of nVertices");
    ...
    if (nCacheSize == 0)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleVCacheClusters: nCacheSize = 0");
    ...
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleVCacheClusters: Invalid selection for vertex cache optimization algorithm");
                                                     ...
                                                     float*              pfEfficiencyOut)
{
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleMeasureCacheEfficiency: Invalid value of nFaces");
    ...
    if (nCacheSize == 0)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleMeasureCacheEfficiency: nCacheSize = 0");
                                              ...
                                              TootleOverdrawOptimizer eOverdrawOptimizer)
{
    ...
    if (nVertices == 0 || nVertices > TOOTLE_MAX_VERTICES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleMeasureOverdraw: nCacheSize = 0");
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleMeasureOverdraw: Invalid value of nFaces");
    ...
    if (nVBStride < 3 * sizeof(float))
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleMeasureOverdraw: nVBStride less than 3*sizeof(float)");
...
static TootleResult ConvertClusterArrayFromFullToCompact(unsigned int* pnID, unsigned int nFaces)
{
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : ConvertClusterArrayFromFullToCompact: nFaces is invalid");
    ...
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : ConvertClusterIDFromFullToCompact: pnID is not a full format");
    ...
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : ConvertClusterArrayFromFullToCompact: nNumCluster > nFaces");
    ...
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : ConvertClusterIDFromFullToCompact: nNumCluster >= TOOTLE_NONE (max int)");
    ...
    if (pnID[ 0 ] != 0)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : ConvertClusterIDFromFullToCompact: pnID[0] != 0");
...
static TootleResult ConvertClusterArrayFromCompactToFull(unsigned int* pnID, unsigned int nFaces)
{
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : ConvertClusterArrayFromCompactToFull: nFaces is invalid");
    ...
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : ConvertClusterArrayFromCompactToFull: pnID is not a compact format");
    ...
    if (nNumCluster > nFaces)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : ConvertClusterArrayFromCompactToFull: nNumCluster > nLength");
    ...
    for (i = 1; i <= nNumCluster; i++)
    {
        if (pnTmp[i] > nFaces)
        {
            logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : ConvertClusterArrayFromCompactToFull: pnTmp[i] > nLength");
            return TOOTLE_INTERNAL_ERROR;
        }
                                                   ...
                                                   unsigned int*       pnVertexRemapOut)
{
    ...
    if (nVertices == 0 || nVertices >= TOOTLE_MAX_VERTICES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVertexMemory: nVertices is invalid");
    ...
    if (nFaces == 0 || nFaces > TOOTLE_MAX_FACES)
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVertexMemory: nFaces is invalid");
    ...
    if (nVBStride < 3 * sizeof(float))
    {
        logger.error("[AMD TOOTLE Debugger] Flag: ERROR - Message : TootleOptimizeVertexMemory: nVBStride is less than 3*sizeof(float)");
    ...
    for (i = 0; i < nFaces3; i++)
    {
        ...
        else
        {
            if (bWarning)
            {
                logger.warning("[AMD TOOTLE Debugger] Flag: WARNING - Message : TootleOptimizeVertexMemory's warning: triangle indices are referencing out-of-bounds vertex buffer");
                bWarning = false;
            }
        }
    }
