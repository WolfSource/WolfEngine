...
HRESULT w_button::on_initialize(const std::shared_ptr<wolf::graphics::w_graphics_device>& pGDevice)
{
	if (!this->icon_path.empty())
	{
		...
		this->icon->set_path(this->icon_path.c_str(), this->icon_is_absolute_path);
		this->icon->set_use_default_size(this->use_default_icon_size);
		this->icon->set_size(this->icon_width, this->icon_height);

		this->icon->set_position_2d(
...
void w_button::render(const std::shared_ptr<wolf::graphics::w_graphics_device>& pGDevice, _In_ float pElapsedTime)
{
	...
	if (this->icon)
	{
		...
		this->icon->set_use_default_size(false);
		this->icon->set_size(this->icon_width, this->icon_height);
