...
static void CALLBACK on_gui_event(_In_ UINT pEvent, _In_ int pControlID, _In_ wolf::gui::w_control* pControl, _In_ void* pUserContext);
...
void w_tab::render(_In_ const std::shared_ptr<wolf::graphics::w_graphics_device>& pGDevice, _In_ float pElapsedTime)
{
	...
	int _x = 0, _y = 0;
	this->buttons.at(_size - 1)->get_position_2d(_x, _y);
	...
	_x = 0; _y = 0;
	this->buttons.at(0)->get_position_2d(_x, _y);
	_enable = _x <= _super::x;
	...
	std::for_each(this->buttons.begin(), this->buttons.end(), [this, _margin](w_button* pButton)
	{
		if (pButton)
		{
			...
			if (_x != INT_MIN && _x != INT_MAX)
			{
				...
				{
					//change position
					pButton->set_position_2d(INT_MAX, _y);
				}
				...
				{
					//change position
					pButton->set_position_2d(INT_MIN, _y);
				}
			}
		}
	});
	...
	_In_ void* pTag)
{
	...
	_super::parent_widget->add_button(this->id + _size,
		...
		pIconPath.c_str(), pIconAbsolutePath,
...
void w_tab::select_next_tab()
{
	...
	for (size_t i = 0; i < this->buttons.size(); ++i)
	{
		...
		if (_button)
		{
			...
			_button->set_position_2d(_x - this->button_width, _y);
		}
	}
}
