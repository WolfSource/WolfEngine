...
HRESULT w_widget::load(_In_ const std::shared_ptr<wolf::graphics::w_graphics_device>& pGDevice,
                       _In_z_ const std::wstring& pControlTextureFilePath)
{
	this->_gDevice = pGDevice;

    if (!pControlTextureFilePath.empty())
    {
        if(w_texture::load_to_shared_textures(this->_gDevice, pControlTextureFilePath, &this->_texture) == S_FALSE)
        {
            V(S_FALSE, L"loading texture of gui: " + pControlTextureFilePath + L" the default texture will be used.", this->name, 2);
    ...
    _param.sampler_info = this->_texture->get_descriptor_info() ;
    
    std::vector<w_shader_binding_param> _shader_binding_params = { _param };
    
    auto _hr = w_shader::load_to_shared_shaders(pGDevice,
                                                "w_gui",
    ...
    if (_hr == S_FALSE)
    {
        V(S_FALSE, "loading shader of gui", this->name, 3);
        return S_FALSE;
    }
