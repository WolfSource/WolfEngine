...
namespace wolf
{
	namespace gui
	{
		...
		{
		public:
			W_EXP						w_control(_In_opt_ w_widget* pParent);
			W_EXP	virtual				~w_control();
...
#pragma region Getters
			...
			W_EXP	virtual bool		get_visible() const							{ return this->visible; }
			W_EXP	virtual bool		get_enabled() const							{ return this->enabled; }
			W_EXP	//get the position
			W_EXP	void				get_position_2d(int& pX, int& pY) const		{ pX = this->x; pY = this->y; }
			W_EXP	W_GUI_CONTROL_TYPE	get_type() const							{ return this->type; }
			W_EXP	INT64				get_ID() const								{ return this->id; }
			W_EXP	//get hot key
			W_EXP	UINT				get_hot_key() const							{ return this->hot_key; }
			W_EXP	void*				get_tag() const 							{ return this->tag; }
			W_EXP	w_element*			get_element(_In_ UINT pIndex) const			{ return this->elements[pIndex]; }
			W_EXP	float				get_z_order()								{ return this->z_order; }
            W_EXP	w_widget*			get_parent_widget() const                   { return this->parent_widget; }
            W_EXP	UINT				get_index() const                           { return this->index; }

#pragma endregion
			
#pragma region Setters

            W_EXP void			        set_index(_In_ const UINT pValue)           { this->index = pValue; }
			W_EXP virtual void			set_enabled(_In_ bool pEnabled)				{ this->enabled = pEnabled; }
			W_EXP virtual void			set_visible(_In_ bool pVisible)				{ this->visible = pVisible; }
			W_EXP void					set_ID(_In_ int pID)						{ this->id = pID; }
			W_EXP //set the position
			W_EXP virtual void			set_position_2d(_In_ int pX, _In_ int pY)	{ this->x = pX; this->y = pY; update_rects(); }
			W_EXP void					set_width(UINT pValue)						{ this->width = pValue; update_rects(); }
			W_EXP void					set_height(UINT pValue)						{ this->height = pValue; update_rects(); }
			W_EXP void					set_size(UINT pWidth, UINT pHeight)			{ this->width = pWidth; this->height = pHeight; update_rects();}
			W_EXP //set hot key
			W_EXP void					set_hot_key(_In_ UINT pHotkey)				{ this->hot_key = pHotkey; }
			W_EXP void					set_tag(_In_opt_ void* pUserData)			{ this->tag = pUserData; }
			W_EXP void					set_z_order(_In_ float pValue)				{ this->z_order = pValue; }
			W_EXP 
			W_EXP HRESULT				set_element(_In_ UINT pIndexElement, _In_ w_element* pElement);
