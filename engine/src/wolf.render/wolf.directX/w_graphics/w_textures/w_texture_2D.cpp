	...
	_In_ UINT pBindFlags)
{
	...
	{
		if (S_FALSE == wolf::system::io::get_is_file(_path.c_str()))
		{
		...
		else
		{
			...
			//OutputDebugString(wolf::system::convert::from_utf8(__path).c_str());

			int _width, _height, _comp;
			unsigned char* _rgba = stbi_load(_path.c_str(), &_width, &_height, &_comp, STBI_rgb_alpha);
			if (_rgba)
			{
				if (!pTexture)
				{
...
void w_texture_2D::_stamp_color_probe(_Inout_ UINT* pSrc, _In_ D3D11_TEXTURE2D_DESC pTextureDescription)
{
	...
	for (int y = 0; y < 64; y++)
	{
		for (int x = 0; x < 64; x++)
		{
			...
			float _a = static_cast<float>((1 - _length) * 27);
			float _blk = static_cast<float>(1 - (1 - _length) * 15);
