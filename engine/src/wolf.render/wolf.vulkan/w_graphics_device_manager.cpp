...
ULONG w_graphics_device::release()
{
    ...
    for (size_t i = 0; i < this->output_presentation_windows.size(); ++i)
    {
        ...
        vkFreeCommandBuffers(this->vk_device,
                             ...
                             _output_window->vk_command_buffers.size(),
...
namespace wolf
{
    namespace graphics
    {
        ...
        {
        public:
            w_graphics_device_manager_pimp(_In_ bool pUseWrapMode) : _use_warp_mode(pUseWrapMode), _name("w_graphics_device_manager_pimp")
            {
            ...
            void enumerate_devices(_Inout_  std::vector<std::shared_ptr<w_graphics_device>>& pGraphicsDevices)
            {
                ...
                for(size_t i = 0; i < _gpu_count; ++i)
                {
                    ...
                    auto _win = this->_windows_info.find(i);
                    if (_win !=  this->_windows_info.end())
                    {
                        for(size_t j = 0; j < _win->second.size(); ++j)
                        {
                            ...
                            _create_semaphores(_gDevice, j);
                            _create_command_buffer(_gDevice, j);
                        }
                    }
...
#pragma endregion
        ...
        private:
            ...
            void _create_semaphores(_In_ const std::shared_ptr<w_graphics_device>& pGDevice,
                                    _In_ size_t pOutputPresentationWindowIndex)
            {
                auto _device_name = wolf::system::convert::string_to_wstring(pGDevice->device_name);
                auto _output_presentation_window = &(pGDevice->output_presentation_windows.at(pOutputPresentationWindowIndex));
            ...
            void _create_command_buffer(_In_ const std::shared_ptr<w_graphics_device>& pGDevice,
                                       _In_ size_t pOutputPresentationWindowIndex)
            {
                auto _device_name = wolf::system::convert::string_to_wstring(pGDevice->device_name);
                auto _output_presentation_window = &(pGDevice->output_presentation_windows.at(pOutputPresentationWindowIndex));
                ...
                _command_pool_info.queueFamilyIndex = pGDevice->vk_queue_family_selected_index;
                ...
                for (uint32_t i = 0; i < _output_presentation_window->vk_swap_chain_image_views.size(); ++i)
                {
                    ...
                    _present_to_clear_barrier.dstQueueFamilyIndex = pGDevice->vk_queue_family_selected_support_present_index;
                    ...
                    _clear_to_present_barrier.dstQueueFamilyIndex = pGDevice->vk_queue_family_selected_support_present_index;
...
w_graphics_device_manager::w_graphics_device_manager(bool pUseWrapMode) : _pimp(new w_graphics_device_manager_pimp(pUseWrapMode))
{
