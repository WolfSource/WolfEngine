...
typedef struct w_media_converter_t
{
	struct SwsContext* sws;
	struct SwrContext* swr;
	AVFrame* out_frame;
	uint8_t* out_frame_buf;
	w_video_frame_info	src_video_info;
	w_audio_frame_info	src_audio_info;
	w_video_frame_info	dst_video_info;
	w_audio_frame_info	dst_audio_info;
} w_media_converter_t;

#pragma region static variables

static bool s_register_network = false;
static w_thread_once_flag s_once_flag_init = NULL;
static w_thread_once_flag s_once_flag_mf_fini = NULL;

#pragma endregion

#pragma region static functions

static void s_thread_once_init_job(void)
{
	const char* _trace_info = "s_thread_once_init_job";

	av_register_all();
	avcodec_register_all();

	if (s_register_network)
	{
		int _ret = avformat_network_init();
		if (_ret)
		{
			W_ASSERT_P(false, "avformat_network_init failed! trace info: %s", _trace_info);
		}
	}
	...
	if (_hr != S_OK)
	{
		W_ASSERT_P(false, "MFStartup failed! trace info: %s", _trace_info);
	}
#endif
}
...
static void s_thread_once_mf_fini_job(void)
{
	MFShutdown();
}
...
static void s_update_pts(av* pCodec, int64_t pPacketDts)
{
	if (!pCodec) return;

	//calculate audio pts
	if (pPacketDts == AV_NOPTS_VALUE &&
		pCodec->avframe->opaque &&
		*(uint64_t*)pCodec->avframe->opaque != AV_NOPTS_VALUE)
	{
		pCodec->pts = *(uint64_t*)pCodec->avframe->opaque;
	}
	else if (pPacketDts != AV_NOPTS_VALUE)
	{
		pCodec->pts = pPacketDts;
	}
	else
	{
		pCodec->pts = 0;
	}
	pCodec->pts *= av_q2d(pCodec->avstream->time_base);
}

static void s_update_clock(av* pCodec)
{
	double frame_delay;
	if (pCodec->pts != 0)
	{
...
static void av_fini(av pAV)
{
	...
	if (pAV.avstream)
	{
		avcodec_close(pAV.avstream->codec);
	}
}
...
{
	const char* _trace_info = "w_media_init";
	if (!pMemPool)
	{
	...
	s_register_network = pRegisterNetwork;

	W_RESULT _ret = w_thread_init_once_flag(
		pMemPool,
		&s_once_flag_init);
	if (_ret)
	{
		W_ASSERT_P(false,
			"could not allocate once flag for w_media init! trace info: %s",
			_trace_info);
		return _ret;
	}

#ifdef W_PLATFORM_WIN
	_ret = w_thread_init_once_flag(
		pMemPool,
		&s_once_flag_mf_fini);
	if (_ret)
	{
		W_ASSERT_P(false,
			"could not allocate once flag for w_media fini! trace info: %s",
			_trace_info);
		return _ret;
	}
#endif

	_ret = w_thread_once_call(
		s_once_flag_mf_fini,
		s_thread_once_init_job);
	if (_ret)
	{
		W_ASSERT_P(false,
			"could not call once flag for w_media init! trace info: %s",
			_trace_info);
	}
...
{
	const char* _trace_info = "w_media_open_stream_receiver";
	if (!pMemPool || !pURL)
	{
		W_ASSERT_P(false, "bad args! trace info: %s", _trace_info);
		return W_BAD_ARG;
	...
	{
		w_mem_pool _local_pool_strings = NULL;
		W_RESULT _ret = w_mem_pool_init_from_parent(&_local_pool_strings, &pMemPool);
		if (_ret)
		{
			LOG_P(W_LOG_ERROR,
				"could not allocate for string pool. trace info: %s", pURL, _trace_info);
			return _ret;
		}
	...
	av _av_video = { 0 };
	av _av_audio = { 0 };
	w_video_frame_info_t _video_frame = { 0 };
	w_audio_frame_info_t _audio_frame = { 0 };
...
#ifdef  W_PLATFORM_WIN
	sprintf_s(_buf, MAX_PATH,
#else
	sprintf(_buf,
#endif //  W_PLATFORM_WIN
		"%llu", pSocketTimeOut * 1000);

	av_dict_set(&_av_dic, "stimeout", _buf, 0);//"10000000" 10 second for io socket timeout

	_avformat_ctx = avformat_alloc_context();
	//create an input context
	if (avformat_open_input(&_avformat_ctx, pURL, NULL, &_av_dic) < 0)
	{
		LOG_P(W_LOG_ERROR,
			"openning context for input stream %s. trace info: %s", pURL, _trace_info);
		return _ret;
	}
	...
	{
		LOG_P(W_LOG_ERROR,
			"allocating context for input stream %s. trace info: %s", pURL, _trace_info);
		return _ret;
	}
    ...
    int _video_stream_index = av_find_best_stream(_avformat_ctx, AVMEDIA_TYPE_VIDEO, -1, -1, NULL, 0);
    int _audio_stream_index = av_find_best_stream(_avformat_ctx, AVMEDIA_TYPE_AUDIO, -1, -1, NULL, 0);
	...
	{
		_av_video.avstream = _avformat_ctx->streams[_video_stream_index];
		_av_video.avcodec_ctx = _avformat_ctx->streams[_video_stream_index]->codec;
	}
	if (_audio_stream_index != -1)
	{
		_av_audio.avstream = _avformat_ctx->streams[_audio_stream_index];
		_av_audio.avcodec_ctx = _avformat_ctx->streams[_audio_stream_index]->codec;
	}
	...
	if (_video_stream_index != -1)
	{
		int _codec_id = _av_video.avcodec_ctx->codec_id;
		_av_video.avcodec = avcodec_find_decoder(_codec_id);
		if (!_av_video.avcodec)
		{
			LOG_P(W_LOG_ERROR,
		...
		if (avcodec_open2(_av_video.avcodec_ctx, _av_video.avcodec, NULL) < 0)
		{
			LOG_P(W_LOG_ERROR,
		...
		int _w = _av_video.avcodec_ctx->width;
		int _h = _av_video.avcodec_ctx->height;

		_video_frame.width = _w;
		_video_frame.height = _h;
		_video_frame.format = (int)_av_video.avcodec_ctx->pix_fmt;
	...
	if (_audio_stream_index != -1)
	{
		int _codec_id = _av_audio.avcodec_ctx->codec_id;
		_av_audio.avcodec = avcodec_find_decoder(_av_audio.avcodec_ctx->codec_id);
		if (!_av_audio.avcodec)
		{
			LOG_P(W_LOG_ERROR,
				"could not find decoder for codec id: %d input stream: %s. trace info: %s",
				_codec_id, pURL, _trace_info);
		...
		if (avcodec_open2(_av_audio.avcodec_ctx, _av_audio.avcodec, NULL) < 0)
		{
			LOG_P(W_LOG_ERROR,
				"could not open codec id: %d input stream: %s. trace info: %s",
				_codec_id, pURL, _trace_info);
		...
		_av_audio.avframe = av_frame_alloc();
		if (!_av_audio.avframe)
		{
			LOG_P(W_LOG_ERROR,
		...
		_audio_frame.channel_layout = av_get_default_channel_layout(_av_audio.avcodec_ctx->channels);
		_audio_frame.sample_rate = _av_audio.avcodec_ctx->sample_rate;
		if (_av_audio.avcodec->sample_fmts)
		{
			_audio_frame.format = (int)*_av_audio.avcodec->sample_fmts;
		}
	}

	{
		...
		bool _exit = false;
		double _now;
		double _since_live = w_chrono_now_in_sec();
		while (!_exit)
		{
			av_init_packet(&_packet);
			if (av_read_frame(_avformat_ctx, &_packet) >= 0)
			{
				if (!_connected)
				{
					_connected = true;
					_since_live = w_chrono_now_in_sec();
					pOnConnectionEstablished(pURL);
				}
				...
				if (_packet.stream_index == _video_stream_index)
				{
					...
					if (_ret > 0)
					{
						s_update_pts(&_av_video, _packet.dts);

						if (!_got_packet) continue;

						s_update_clock(&_av_video);

						//pass to convert function
						_video_frame.index++;
						_video_frame.line_size = _av_video.avframe->linesize[0];
				...
				else if (_packet.stream_index == _audio_stream_index)
				{
					_ret = avcodec_decode_audio4(
						_av_audio.avcodec_ctx,
						_av_audio.avframe,
						&_got_packet,
						&_packet);
					if (_ret > 0)
					{
						...
						//pass data to convert function
						_audio_frame.index++;
						_audio_frame.line_size = _av_audio.avframe->linesize[0];
						_audio_frame.nb_samples = _av_audio.avframe->nb_samples;
						_audio_frame.pts = _av_audio.pts;	
						if (pOnAudioFrameRecieved)
						{
							pOnAudioFrameRecieved(&_audio_frame, _av_audio.avframe->data[0]);
						}
					}
				}
			}
			else
			{
				...
				{
					pOnConnectionLost(pURL);
				}

				_now = w_chrono_now_in_sec();
				if (_now - _since_live >= pStreamTimeOut)
				{
					_exit = true;
				}
			}
		...
		if (pOnConnectionClosed)
		{
			pOnConnectionClosed(pURL);
		}
	}
...
exit:
	...
	if (_avformat_ctx)
	{
		avio_close(_avformat_ctx->pb);
		avformat_free_context(_avformat_ctx);
	}
...
{
	const char* _trace_info = "w_media_convert_video";
	if (!pMemPool || !pSrcFrameInfo || !pDstFrameInfo || !pSrcFrameBuffer)
	{
		W_ASSERT_P(false, "bad args! trace info: %s", _trace_info);
		return -1;
	}

	if (!pMediaConverter || !(*pMediaConverter))
	{
		w_media_converter _cnv = (w_media_converter)w_malloc(pMemPool, sizeof(w_media_converter_t));
		if (!_cnv)
		{
			W_ASSERT_P(false, "could not allocate memory for media converter! trace info: %s", _trace_info);
			return -1;
		}

		//create video converter
		if (pSrcFrameInfo->width > 0 && pSrcFrameInfo->height > 0 && pSrcFrameInfo->format > 0 &&
			pDstFrameInfo->width > 0 && pDstFrameInfo->height > 0 && pDstFrameInfo->format > 0)
		{
			size_t _size = sizeof(w_video_frame_info);
			_cnv->src_video_info = w_malloc(pMemPool, _size);
			if (!_cnv->src_video_info)
			{
				W_ASSERT_P(false, "could not allocate memory for src_video_info! trace info: %s",
					_trace_info);
				return -1;
			}
			_cnv->dst_video_info = w_malloc(pMemPool, _size);
			if (!_cnv->dst_video_info)
			{
				W_ASSERT_P(false, "could not allocate memory for dst_video_info! trace info: %s",
					_trace_info);
				return -1;
			}

			//copy media converter infos
			memcpy(_cnv->src_video_info, pSrcFrameInfo, _size);
			memcpy(_cnv->dst_video_info, pDstFrameInfo, _size);

			//create sws scale
			_cnv->sws = sws_getContext(
				_cnv->src_video_info->width,
				_cnv->src_video_info->height,
				(enum AVPixelFormat)_cnv->src_video_info->format,
				_cnv->dst_video_info->width,
				_cnv->dst_video_info->height,
				(enum AVPixelFormat)_cnv->dst_video_info->format,
				SWS_BICUBIC,
				NULL,
				NULL,
				NULL);
			if (!_cnv->sws)
			{
				W_ASSERT_P(false, "could not create sws! trace info: %s", _trace_info);
				return -1;
			}

			//allocate output frame
			_cnv->out_frame = av_frame_alloc();
			if (!_cnv->out_frame)
			{
				W_ASSERT_P(false, "could not allocate av frame! trace info: %s", _trace_info);
				return -1;
			}

			//allocate output frame buffer
			int _buffer_size = avpicture_get_size(
				(enum AVPixelFormat)_cnv->dst_video_info->format,
				_cnv->dst_video_info->width,
				_cnv->dst_video_info->height);

			_cnv->out_frame_buf = (uint8_t*)(av_malloc(_buffer_size));
			if (!_cnv->out_frame_buf)
			{
				W_ASSERT_P(false, "could not allocate av frame buffer! trace info: %s", _trace_info);
				return -1;
			}
			if (avpicture_fill(
				(AVPicture*)_cnv->out_frame,
				_cnv->out_frame_buf,
				_cnv->dst_video_info->format,
				_cnv->dst_video_info->width,
				_cnv->dst_video_info->height) < 0)
			{
				W_ASSERT_P(false, "could not fill av frame buffer! trace info: %s", _trace_info);
				return -1;
			}
		}

		*pMediaConverter = _cnv;
	}

	//convert video
	const uint8_t* const _src_slice[1] = { pSrcFrameBuffer };
	uint8_t* const _dst_slice[1] = { *pDstFrameBuffer };
	const int _src_stride[1] = { pSrcFrameInfo->line_size };
	...
	return sws_scale(
		...
		_src_stride,
		0,
		(*pMediaConverter)->src_video_info->height,
		_dst_slice,
		(*pMediaConverter)->out_frame->linesize);
}

W_RESULT w_media_convert_audio(
	_In_ w_mem_pool pMemPool,
	...
	_Inout_ w_media_converter* pMediaConverter)
{
	const char* _trace_info = "w_media_convert_audio";
	if (!pMemPool || !pSrcFrameInfo || !pDstFrameInfo || !pSrcFrameBuffer)
	{
		W_ASSERT_P(false, "bad args! trace info: %s", _trace_info);
		return -1;
	...
	if (!pMediaConverter || !(*pMediaConverter))
	{
		...
		//create video converter
		if (pSrcFrameInfo->channel_layout > 0 && pSrcFrameInfo->sample_rate > 0 && pSrcFrameInfo->format > 0 &&
			pDstFrameInfo->channel_layout > 0 && pDstFrameInfo->sample_rate > 0 && pDstFrameInfo->format > 0)
		{
			size_t _size = sizeof(w_audio_frame_info);
			_cnv->src_video_info = w_malloc(pMemPool, _size);
			if (!_cnv->src_video_info)
			{
				W_ASSERT_P(false, "could not allocate memory for src_video_info! trace info: %s",
			...
			//copy media converter infos
			memcpy(_cnv->src_audio_info, pSrcFrameInfo, _size);
			memcpy(_cnv->dst_audio_info, pDstFrameInfo, _size);

			//Create swr for converting audio
			_cnv->swr = swr_alloc_set_opts(
				_cnv->swr,
				_cnv->dst_audio_info->channel_layout,
				(enum AVSampleFormat)_cnv->dst_audio_info->format,
				_cnv->dst_audio_info->sample_rate,
				_cnv->src_audio_info->channel_layout,
				(enum AVSampleFormat)_cnv->src_audio_info->format,
				_cnv->src_audio_info->sample_rate,
				0,
				NULL);
			if (!_cnv->swr)
			{
				W_ASSERT_P(false, "could not create swr! trace info: %s",
					_trace_info);
				return W_FAILURE;
			}
			swr_init(_cnv->swr);
	...
	return swr_convert(
		(*pMediaConverter)->swr,
		pDstFrameBuffer,
		pDstFrameInfo->line_size,
		(const uint8_t**)pSrcFrameBuffer,
		pSrcFrameInfo->nb_samples);
}
