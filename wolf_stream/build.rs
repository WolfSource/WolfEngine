...
fn main() {
    ...
    // create deps folder
    make_folder(format!("{}/deps/", current_dir).as_str());
    ...
    for (k, mut v) in git_sources {
        ...
        if build {
            ...
            assert!(build_ret.is_ok(), "{:?}", build_ret);
        }
    ...
    for iter in lib_paths {
        ...
        //TODO: -llive555 & -lffmpeg is not exist, we already use link dependencies
        //we should write another library namely wolf_build
        // if iter.1 .1 {
...
fn find_msvc_path(p_batch_path: &str, p_shell_type: &str) -> Result<String> {
    ...
    let shell_command = format!("{} > {}/output", batch_file, p_batch_path);
