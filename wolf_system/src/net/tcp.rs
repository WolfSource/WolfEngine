...
pub async fn server(
    ...
    loop {
        ...
        if p_config.accept_timeout_in_secs > 0.0 {
            //try for accept incomming session within specific timout
            _res = tokio::select! {
                res1 = timeout_for_accept(p_config.accept_timeout_in_secs) =>
                {
                    res1
                },
...
pub async fn client(
    ...
    if p_config.tls {
        ...
        let root_cert_store = load_root_ca(p_config.tls_ca_path).await?;
        // load tls config
    ...
    let _ = tokio::spawn(async move {
        ...
        let on_close_connection = OnSocketCallback::new(Box::new(
            |p_socket_address: &SocketAddr| -> anyhow::Result<()> {
                ...
                let _ = CHANNEL_MUTEX.lock().0.send(true).map_err(|e| {
