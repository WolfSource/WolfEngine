...
pub const LZ4F_errorCodes_LZ4F_ERROR_maxCode: LZ4F_errorCodes = 20;
pub const LZ4F_errorCodes__LZ4F_dummy_error_enum_for_c89_never_used: LZ4F_errorCodes = 21;
pub type LZ4F_errorCodes = ::std::os::raw::c_uint;
extern "C" {
    pub fn LZ4F_getErrorCode(functionResult: size_t) -> LZ4F_errorCodes;
}
extern "C" {
    pub fn LZ4F_getBlockSize(arg1: ::std::os::raw::c_uint) -> size_t;
}
#[repr(C)]
#[derive(Debug, Copy, Clone)]
...
extern "C" {
    #[doc = " LZ4_createCDict() :"]
    #[doc = "  When compressing multiple messages / blocks using the same dictionary, it's recommended to load it just once."]
    #[doc = "  LZ4_createCDict() will create a digested dictionary, ready to start future compression operations without startup delay."]
    #[doc = "  LZ4_CDict can be created once and shared by multiple threads concurrently, since its usage is read-only."]
    #[doc = " `dictBuffer` can be released after LZ4_CDict creation, since its content is copied within CDict"]
...
extern "C" {
    ...
    #[doc = "  Compress an entire srcBuffer into a valid LZ4 frame using a digested Dictionary."]
    #[doc = "  cctx must point to a context created by LZ4F_createCompressionContext()."]
    #[doc = "  If cdict==NULL, compress without a dictionary."]
    #[doc = "  dstBuffer MUST be >= LZ4F_compressFrameBound(srcSize, preferencesPtr)."]
    #[doc = "  If this condition is not respected, function will fail (@return an errorCode)."]
    #[doc = "  The LZ4F_preferences_t structure is optional : you may provide NULL as argument,"]
    #[doc = "  but it's not recommended, as it's the only way to provide dictID in the frame header."]
    #[doc = " @return : number of bytes written into dstBuffer."]
    #[doc = "           or an error code if it fails (can be tested using LZ4F_isError())"]
...
extern "C" {
    ...
    #[doc = "  Inits streaming dictionary compression, and writes the frame header into dstBuffer."]
    #[doc = "  dstCapacity must be >= LZ4F_HEADER_SIZE_MAX bytes."]
    #[doc = " `prefsPtr` is optional : you may provide NULL as argument,"]
    #[doc = "  however, it's the only way to provide dictID in the frame header."]
    #[doc = " @return : number of bytes written into dstBuffer for the header,"]
    #[doc = "           or an error code (which can be tested using LZ4F_isError())"]
...
extern "C" {
    ...
    #[doc = "  Same as LZ4F_decompress(), using a predefined dictionary."]
    #[doc = "  Dictionary is used \"in place\", without any preprocessing."]
    #[doc = "  It must remain accessible throughout the entire frame decoding."]
