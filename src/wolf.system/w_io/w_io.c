    ...
    _In_z_ const char* pPath)
{
    ...
    char* _dst_str = (char*)w_malloc(pMemPool, _index);
    if (!_dst_str)
    {
        W_ASSERT_P(false, "could not allocate memory. trace info: %s", _trace_info);
        return NULL;
    }
    ...
    _In_z_ const char* pPath)
{
    ...
    const char* _base_name = w_io_file_get_basename_from_path(
        pMemPool,
        pPath);
    if (!_base_name)
    {
        W_ASSERT_P(false, "memory for base name was not allocated. trace info: %s", _trace_info);
        return NULL;
    }

    return w_io_file_get_name_from_path(pMemPool, _base_name);
}
    ...
    _In_ w_file pFile)
{
    ...
    if (!_name)
    {
        W_ASSERT_P(false, "strdup failed. trace info: %s", _trace_info);
        return NULL;
    }
    return w_io_file_get_basename_without_extension_from_path(pMemPool, _name);
}
    ...
    _In_ w_file pFile)
{
    ...
    if (!_file)
    {
        W_ASSERT_P(false, "failed to get file info. trace info: %s", _trace_info);
        return W_FAILURE;
    }
    ...
    if (!_pool)
    {
        W_ASSERT_P(false, "failed to get apr pool. trace info: %s", _trace_info);
        return W_FAILURE;
    }

    return apr_file_remove(_file->fname, _pool);
}

W_RESULT w_io_dir_get_current(_Inout_ w_mem_pool pMemPool, _Inout_ char** pDir)
{
    const char* _trace_info = "w_io_dir_get_current";
    ...
    if (_tmp)
    {
        ...
        if (_len)
        {
            ...
            if (!_dir)
            {
                W_ASSERT_P(false, "failed to allocate memory. trace info: %s", 
                    _trace_info);
                return W_FAILURE;
            }
    ...
    _In_z_ const char* pPath)
{
    ...
    if (!_pool)
    {
        W_ASSERT_P(false, "could not get memory pool. trace info: %s", _trace_info);
        return W_FAILURE;
    }

    //open a directory
    apr_dir_t* _dir = NULL;
    W_RESULT _ret = apr_dir_open(&_dir, pPath, _pool);
    ...
    _In_z_ const char* pPath)
{
    ...
    if (!_dst)
    {
        W_ASSERT_P(false, "could no allocate memory. trace info: %s", _trace_info);
        return NULL;
    }
    ...
    _In_z_ const char* pPath)
{
    ...
    if (!_pool)
    {
        W_ASSERT_P(false, "could not get apr pool. trace info: %s", _trace_info);
        return W_FAILURE;
    }

    //create a directory
    return apr_dir_make_recursive(pPath, APR_OS_DEFAULT, _pool);
}

W_RESULT	w_io_utf8_to_ucs2(
    _In_ char* pIn,
    _In_ size_t* pInLen,
    _Inout_ uint16_t* pOut,
    _Inout_ size_t* pOutLen)
{
        ...
        else
        {
            ...
            else
            {
                ...
                */
                ...
                newch = ch & ~mask;
                eating = expect + 1;
                if (*pInLen <= expect) return APR_BADARG;
                /* 
                ...
                */
                if (expect == 1) 
                {
                    if (!(newch & 0036)) return APR_BADARG;
                }
                else 
                {
                    // Reject values of excessive leading 0 bits 
                    ...
                    if (expect == 2) {
                        ...
                        */
                        ...
                        if (newch > 4) return APR_BADARG;
                        if (newch == 4 && ((unsigned char)*pIn & 0060)) return APR_BADARG;
                    }
                }
                /* 
                ...
                */
                if (*pOutLen < (apr_size_t)(expect > 2) + 1) break; // buffer full
                while (expect--)
                {
    ...
    _Inout_ size_t* pOutLen)
{
    ...
    while (*pInLen && *pOutLen)
    {
        ...
        else
        {
            ...
            *pInLen -= ( need > ((apr_size_t)2) ) + (apr_size_t)1;
            *pOutLen -= need + 1;
            /*
