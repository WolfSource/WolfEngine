 ...
 */
    ...
    //we need to implement it in apr 1.7
#include <assert.h>
#include <apr_ring.h>
...
static apr_status_t thread_pool_cleanup(void *me)
{
    ...
    w_thread_pool_idle_wait_set(_myself, 0);
...
//NOTE: This function is not thread safe by itself. Caller should hold the lock
static apr_thread_pool_task_t* task_new(
 ...
 * NOTE: This function is not thread safe by itself. Caller should hold the lock
 */
static apr_thread_pool_task_t* add_if_empty(
    apr_thread_pool_t* me,
...
*/
static apr_status_t schedule_task(
    apr_thread_pool_t *me,
    apr_thread_start_t func, 
    void *param,
    void *owner, 
...
static apr_status_t remove_scheduled_tasks(
    apr_thread_pool_t* me,
    void* owner)
{
    apr_thread_pool_task_t* t_loc;
 ...
 */
static struct apr_thread_list_elt* trim_threads(
    apr_thread_pool_t* me,
    apr_size_t* cnt,
    int idle)
{
    ...
    _In_ size_t pMaxThreads)
{
    ...
    *pThreadPool = NULL;
    tp = apr_pcalloc(_pool, sizeof(apr_thread_pool_t));
     ...
     */
    _ret = apr_pool_create(&tp->pool, _pool);
    if (APR_SUCCESS != _ret)
    {
        return _ret;
    }
    _ret = thread_pool_construct(tp, pInitThreads, pMaxThreads);
    if (APR_SUCCESS != _ret)
    {
        return _ret;
    }
    ...
    while (pInitThreads) 
    {
        ...
        _ret = apr_thread_create(&t, NULL, thread_pool_func, tp, tp->pool);
    ...
    _In_opt_ void* pOwner)
{
    ...
    return add_task(pThreadPool, pThreadJob, pParam, pPriority, 1, pOwner);
}
    ...
    _In_opt_ void* pOwner)
{
    ...
    return schedule_task(pThreadPool, pThreadJob, pParam, pOwner, pTime);
}
    ...
    _In_ void* pOwner)
{
    ...
    return add_task(pThreadPool, pThreadJob, pParam, pPriority, 0, pOwner);
}
...
W_RESULT w_thread_pool_tasks_cancel(_In_ w_thread_pool pThreadPool, _In_ void* pOwner)
{
    ...
    apr_status_t _ret = APR_SUCCESS;
    apr_thread_mutex_lock(pThreadPool->lock);
    if (pThreadPool->task_cnt > 0) 
    {
        _ret = remove_tasks(pThreadPool, pOwner);
    }
    if (pThreadPool->scheduled_task_cnt > 0) 
    {
        _ret = remove_scheduled_tasks(pThreadPool, pOwner);
    }
    apr_thread_mutex_unlock(pThreadPool->lock);
    wait_on_busy_threads(pThreadPool, pOwner);
...
size_t w_thread_pool_scheduled_tasks_count(_In_ w_thread_pool pThreadPool)
{
    if (pThreadPool)
    {
        return pThreadPool->scheduled_task_cnt;
    }
    return 0;
}
...
size_t w_thread_pool_busy_count(_In_ w_thread_pool pThreadPool)
{
    if (pThreadPool)
    {
        return pThreadPool->thd_cnt - pThreadPool->idle_cnt;
    }
    return 0;
}
    ...
    _In_ size_t pCount)
{
    if (pThreadPool)
    {
        pThreadPool->idle_max = pCount;
        pCount = trim_idle_threads(pThreadPool, pCount);
        return pCount;
    }
    return 0;
}
...
W_RESULT w_thread_pool_fini(_Inout_ w_thread_pool pThreadPool)
{
    if (pThreadPool)
    {
        apr_pool_destroy(pThreadPool->pool);
        return APR_SUCCESS;
    }
    return APR_BADARG;
}
...
size_t w_thread_pool_threads_idle_timeout_count(_In_ w_thread_pool pThreadPool)
{
    if (pThreadPool)
    {
        return pThreadPool->thd_timed_out;
    }
    return 0;
}
    ...
    _In_ size_t pCount)
{
    ...
    pThreadPool->thd_max = pCount;
    if (0 == pCount || pThreadPool->thd_cnt <= pCount)
    {
        return 0;
    }

    _n = pThreadPool->thd_cnt - pCount;
    if (_n >= pThreadPool->idle_cnt)
    {
        trim_busy_threads(pThreadPool, _n - pThreadPool->idle_cnt);
        trim_idle_threads(pThreadPool, 0);
    }
    else 
    {
        trim_idle_threads(pThreadPool, pThreadPool->idle_cnt - _n);
    }
    return _n;
}
    ...
    _In_ int64_t pTimeout)
{
    ...
    oldtime = pThreadPool->idle_wait;
    pThreadPool->idle_wait = pTimeout;
    ...
    _In_ size_t pValue)
{
    ...
    apr_size_t ov;
    ov = pThreadPool->threshold;
    pThreadPool->threshold = pValue;
    return ov;
}
    ...
    _In_ void** pOwner)
{
    ...
    rv = apr_thread_data_get(&data, "apr_thread_pool_task", pThread);
    if (rv != APR_SUCCESS)
    {
        return rv;
    }
