...
w_tree_list::w_tree_list(_In_opt_ w_widget* pParent) : _super(pParent),
text_margin_x(2),
text_margin_y(25),
icon_margin_x(0),
icon_margin_y(0),
...
tree_list_text_color(w_color::from_hex(w_color::BLACK)),
tree_list_text_selected_color(w_color::from_hex(w_color::WHITE)),
tree_list_text_disabled_color(w_color::from_hex(w_color::GRAY))
{
	_super::type = W_GUI_CONTROL_LISTBOX;
	_super::item_height = 35;
	_super::set_item_selected_rectangle_margin(4, 15);
}

w_tree_list::~w_tree_list()
{
	remove_all_items();
}

HRESULT w_tree_list::add_item(_In_z_ const std::wstring& pText, _In_ const std::vector<std::wstring> pChilds,
	_In_z_ const std::wstring& pIconPath, _In_ bool pIsAbsolutePath, _In_opt_  void* pData)
{
	auto _widget_item = new w_list_widget_item();
	if (!_widget_item) return E_OUTOFMEMORY;

	auto _new_tree_list_item = new (std::nothrow) w_tree_list_item();
	if (!_new_tree_list_item) return E_OUTOFMEMORY;

	_new_tree_list_item->text = pText;
	_new_tree_list_item->icon_path = pIconPath;
	_new_tree_list_item->data = pData;
	...
	_label->set_label_color(this->tree_list_text_color);
	_label->set_current_color_state(this->tree_list_text_color);
	_label->set_label_focused_color(this->tree_list_text_selected_color);
	_label->set_label_disabled_color(this->tree_list_text_disabled_color);
	_label->set_force_use_current_color_state(true);

	//add to list box
	this->list_items.push_back(_new_tree_list_item);

	
	//add a rectangle as base layer
	auto _base_layer = new w_color_quad(_super::parent_widget);
	_base_layer->set_color(w_color::from_hex(w_color::RED));
	_base_layer->set_size(this->width - 35, this->item_height - 10);
	_base_layer->on_initialize(_super::gDevice);
	
	//add to list widget
	_widget_item->gpu_controls.push_back(std::make_tuple(_base_layer, 0, 25));
	//add to list widget
...
HRESULT w_tree_list::insert_item(int pIndex, _In_z_ const std::wstring& pText, _In_z_ const std::wstring& pIconPath, _In_ bool pIsAbsolutePath, _In_opt_ void* pData)
{
	//auto _widget_item = new w_list_widget_item();
	//if (!_widget_item) return E_OUTOFMEMORY;
...
void w_tree_list::render(const std::shared_ptr<wolf::graphics::w_graphics_device>& pGDevice, _In_ float pElapsedTime)
{
...
w_tree_list_item* w_tree_list::get_tree_list_item(_In_ int nIndex) const
{
